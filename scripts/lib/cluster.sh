...
function load_cluster_details() {
  ...
  DESCRIBE_CLUSTER_OP=$(nholuongut eks describe-cluster --name "$CLUSTER_NAME" --region "$REGION" $ENDPOINT_FLAG)
...
function up-test-cluster() {
    ...
    if [[ "$RUN_BOTTLEROCKET_TEST" == true ]]; then
        ...
        AMI_ID=`nholuongut ssm get-parameter --name /nholuongut/service/eks/optimized-ami/${EKS_CLUSTER_VERSION}/amazon-linux-2/recommended/image_id --region us-west-2 --query "Parameter.Value" --output text`
...
function up-kops-cluster {
    KOPS_S3_BUCKET=kops-cni-test-eks-$nholuongut_ACCOUNT_ID
    echo "Using $KOPS_S3_BUCKET as kops state store"
    if ! nholuongut s3api head-bucket --bucket $KOPS_S3_BUCKET 2>/dev/null; then
        nholuongut s3api create-bucket --bucket $KOPS_S3_BUCKET --region $nholuongut_DEFAULT_REGION --create-bucket-configuration LocationConstraint=$nholuongut_DEFAULT_REGION
    fi
    ...
    HOST_IMAGE_SSM_PARAMETER="ssm:/nholuongut/service/canonical/ubuntu/server/20.04/stable/current/amd64/hvm/ebs-gp2/ami-id"
    ...
    # Using Ubuntu 20.04 because of https://github.com/nholuongut/amazon-vpc-cni-k8s/issues/2103
    $KOPS_BIN create cluster \
    --cloud nholuongut \
    --zones ${nholuongut_DEFAULT_REGION}a,${nholuongut_DEFAULT_REGION}b \
    ...
    kubectl delete deployment nholuongut-node-termination-handler -n kube-system --wait=true

    kubectl apply -f https://raw.githubusercontent.com/nholuongut/amazon-vpc-cni-k8s/${MANIFEST_CNI_VERSION}/config/master/cni-metrics-helper.yaml
}

function down-kops-cluster {
    KOPS_BIN=~/kops_bin/kops
    $KOPS_BIN delete cluster --name ${CLUSTER_NAME} --yes
    nholuongut s3 rm ${KOPS_STATE_STORE} --recursive
    nholuongut s3 rb ${KOPS_STATE_STORE} --region $nholuongut_DEFAULT_REGION
}
