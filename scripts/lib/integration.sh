function run_kops_conformance() {
    ...
    sleep 5
    while [[ $(kubectl describe ds nholuongut-node -n=kube-system | grep "Available Pods: 0") ]]
    do
        sleep 5
    ...
    sleep 240 #Workaround to avoid ENI leakage during cluster deletion: https://github.com/nholuongut/amazon-vpc-cni-k8s/issues/1223
}
