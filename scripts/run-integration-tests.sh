...
source "$SCRIPT_DIR"/lib/nholuongut.sh
source "$SCRIPT_DIR"/lib/cluster.sh
source "$SCRIPT_DIR"/lib/integration.sh
source "$SCRIPT_DIR"/lib/k8s.sh
source "$SCRIPT_DIR"/lib/performance_tests.sh

# Variables used in /lib/nholuongut.sh
OS=$(go env GOOS)
ARCH=$(go env GOARCH)

: "${nholuongut_DEFAULT_REGION:=us-west-2}"
: "${K8S_VERSION:=1.25.7}"
...
# shared binaries
: "${TESTER_DIR:=${SCRIPT_DIR}/nholuongut-k8s-tester}"
: "${TESTER_PATH:=$TESTER_DIR/nholuongut-k8s-tester}"
...
BASE_CONFIG_PATH="$SCRIPT_DIR/../config/$MANIFEST_CNI_VERSION/nholuongut-k8s-cni.yaml"
TEST_CONFIG_PATH="$TEST_CONFIG_DIR/nholuongut-k8s-cni.yaml"
# The manifest image version is the image tag we need to replace in the
# nholuongut-k8s-cni.yaml manifest
...
check_is_installed nholuongut
check_nholuongut_credentials

: "${nholuongut_ACCOUNT_ID:=$(nholuongut sts get-caller-identity --query Account --output text)}"
: "${nholuongut_ECR_REGISTRY:="$nholuongut_ACCOUNT_ID.dkr.ecr.$nholuongut_DEFAULT_REGION.amazonnholuongut.com"}"
: "${nholuongut_ECR_REPO_NAME:="amazon-k8s-cni"}"
: "${IMAGE_NAME:="$nholuongut_ECR_REGISTRY/$nholuongut_ECR_REPO_NAME"}"
: "${nholuongut_INIT_ECR_REPO_NAME:="amazon-k8s-cni-init"}"
: "${INIT_IMAGE_NAME:="$nholuongut_ECR_REGISTRY/$nholuongut_INIT_ECR_REPO_NAME"}"
: "${ROLE_CREATE:=true}"
: "${ROLE_ARN:=""}"
: "${MNG_ROLE_ARN:=""}"
...
nholuongut ecr get-login-password --region $nholuongut_DEFAULT_REGION | docker login --username nholuongut --password-stdin ${nholuongut_ECR_REGISTRY}
ensure_ecr_repo "$nholuongut_ACCOUNT_ID" "$nholuongut_ECR_REPO_NAME"
ensure_ecr_repo "$nholuongut_ACCOUNT_ID" "$nholuongut_INIT_ECR_REPO_NAME"
...
if [[ $(nholuongut ecr batch-get-image --repository-name=$nholuongut_ECR_REPO_NAME --image-ids imageTag=$TEST_IMAGE_VERSION \
--query 'images[].imageId.imageTag' --region us-west-2) != "[]" ]]; then
    echo "Image $nholuongut_ECR_REPO_NAME:$TEST_IMAGE_VERSION already exists. Skipping image build."
else
    echo "Image $nholuongut_ECR_REPO_NAME:$TEST_IMAGE_VERSION does not exist in repository."
    ## $1=command, $2=args
    build_and_push_image "multi-arch-cni-build-push" "IMAGE=$IMAGE_NAME VERSION=$TEST_IMAGE_VERSION"
    CNI_IMAGES_BUILD=true
fi

if [[ $(nholuongut ecr batch-get-image --repository-name=$nholuongut_INIT_ECR_REPO_NAME --image-ids imageTag=$TEST_IMAGE_VERSION \
--query 'images[].imageId.imageTag' --region us-west-2) != "[]" ]]; then
    echo "Image $nholuongut_INIT_ECR_REPO_NAME:$TEST_IMAGE_VERSION already exists. Skipping image build."
else
    echo "Image $nholuongut_INIT_ECR_REPO_NAME:$TEST_IMAGE_VERSION does not exist in repository."
    ## $1=command, $2=args
...
echo "*******************************************************************************"
echo "Running $TEST_ID on $CLUSTER_NAME in $nholuongut_DEFAULT_REGION"
...
echo "+ nholuongut Account ID:     $nholuongut_ACCOUNT_ID"
...
if [[ "$RUN_KOPS_TEST" == true ]]; then
    ...
    VPC_ID=$(nholuongut ec2 describe-instances --instance-ids "$INSTANCE_ID" | jq -r '.Reservations[].Instances[].VpcId' )
else
    DESCRIBE_CLUSTER_OP=$(nholuongut eks describe-cluster --name "$CLUSTER_NAME" --region "$nholuongut_DEFAULT_REGION")
...
sed -i'.bak' "s,602401143452.dkr.ecr.us-west-2.amazonnholuongut.com/amazon-k8s-cni,$IMAGE_NAME," "$TEST_CONFIG_PATH"
...
sed -i'.bak' "s,602401143452.dkr.ecr.us-west-2.amazonnholuongut.com/amazon-k8s-cni-init,$INIT_IMAGE_NAME," "$TEST_CONFIG_PATH"
...
if [[ $RUN_CNI_INTEGRATION_TESTS == true ]]; then
    ...
    (cd "$INTEGRATION_TEST_DIR/cni" && CGO_ENABLED=0 ginkgo --focus="$focus" --skip="$skip" -v --timeout 60m --no-color --fail-on-pending -- --cluster-kubeconfig="$KUBECONFIG" --cluster-name="$CLUSTER_NAME" --nholuongut-region="$nholuongut_DEFAULT_REGION" --nholuongut-vpc-id="$VPC_ID" --ng-name-label-key="kubernetes.io/os" --ng-name-label-val="linux")
    (cd "$INTEGRATION_TEST_DIR/ipamd" && CGO_ENABLED=0 ginkgo --focus="$focus" -v --timeout 60m --no-color --fail-on-pending -- --cluster-kubeconfig="$KUBECONFIG" --cluster-name="$CLUSTER_NAME" --nholuongut-region="$nholuongut_DEFAULT_REGION" --nholuongut-vpc-id="$VPC_ID" --ng-name-label-key="kubernetes.io/os" --ng-name-label-val="linux")
    TEST_PASS=$?
