...
import (
	...
	"github.com/nholuongut/amazon-vpc-cni-k8s/pkg/eniconfig"
	"github.com/nholuongut/amazon-vpc-cni-k8s/pkg/ipamd/datastore"
	"github.com/nholuongut/amazon-vpc-cni-k8s/pkg/k8sapi"
	"github.com/nholuongut/amazon-vpc-cni-k8s/pkg/networkutils"
	"github.com/nholuongut/amazon-vpc-cni-k8s/pkg/utils/cniutils"
	...
	rcv1alpha1 "github.com/nholuongut/amazon-vpc-resource-controller-k8s/apis/vpcresources/v1alpha1"
)
...
const (
	...
	envCustomNetworkCfg = "nholuongut_VPC_K8S_CNI_CUSTOM_NETWORK_CFG"
	...
	envManageENIsNonSchedulable = "nholuongut_MANAGE_ENIS_NON_SCHEDULABLE"
	...
	eniNoManageTagKey = "node.k8s.amazonnholuongut.com/no_manage"
	...
	envBackingStorePath     = "nholuongut_VPC_K8S_CNI_BACKING_STORE"
	defaultBackingStorePath = "/var/run/nholuongut-node/ipam.json"
	...
	// envAnnotatePodIP is used to annotate[vpc.amazonnholuongut.com/pod-ips] pod's with IPs
	...
	// nholuongut error codes for insufficient IP address scenario
...
type IPAMContext struct {
	nholuongutClient                 nholuongututils.APIs
...
func (c *IPAMContext) setUnmanagedENIs(tagMap map[string]nholuongututils.TagMap) {
	if len(tagMap) == 0 {
		return
	}
	...
	for eniID, tags := range tagMap {
		...
		} else if _, found := tags[eniNodeTagKey]; found && tags[eniNodeTagKey] == c.nholuongutClient.GetInstanceID() {
			continue
		} else if c.enableManageUntaggedMode {
			continue
		}

		if eniID == c.nholuongutClient.GetPrimaryENI() {
	...
	c.nholuongutClient.SetUnmanagedENIs(unmanagedENIlist)
}
...
func containsInsufficientCIDRsOrSubnetIPs(err error) bool {
	var nholuongutErr nholuonguterr.Error
	...
	if errors.As(err, &nholuongutErr) {
		log.Debugf("Insufficient IP Addresses due to: %v\n", nholuongutErr.Code())
		return nholuongutErr.Code() == INSUFFICIENT_CIDR_BLOCKS || nholuongutErr.Code() == INSUFFICIENT_FREE_IP_SUBNET
	}
	return false
}
...
func New(k8sClient client.Client) (*IPAMContext, error) {
	...
	client, err := nholuongututils.New(c.useSubnetDiscovery, c.useCustomNetworking, disableLeakedENICleanup(), c.enableIPv4, c.enableIPv6)
	if err != nil {
		return nil, errors.Wrap(err, "ipamd: can not initialize with nholuongut SDK interface")
	}
	c.nholuongutClient = client
	...
	c.numNetworkCards = len(c.nholuongutClient.GetNetworkCards())
	...
	err = c.nholuongutClient.FetchInstanceTypeLimits()
	if err != nil {
		log.Errorf("Failed to get ENI limits from file:vpc_ip_limits or EC2 for %s", c.nholuongutClient.GetInstanceType())
		return nil, err
	}
	...
	c.nholuongutClient.InitCachedPrefixDelegation(c.enablePrefixDelegation)
...
func (c *IPAMContext) nodeInit() error {
	...
	primaryV4IP := c.nholuongutClient.GetLocalIPv4()
	...
	if c.enableIPv4 {
		...
		vpcV4CIDRs, err = c.nholuongutClient.GetVPCIPv4CIDRs()
		if err != nil {
			return err
		}
	}

	primaryENIMac := c.nholuongutClient.GetPrimaryENImac()
	...
	err = c.networkClient.CleanUpStalenholuongutChains(c.enableIPv4, c.enableIPv6)
	if err != nil {
		// We should not error if clean up fails since these chains don't affect the rules
		log.Debugf("Failed to clean up stale nholuongut chains: %v", err)
	}

	metadataResult, err := c.nholuongutClient.DescribeAllENIs()
	if err != nil {
	...
	c.nholuongutClient.SetMultiCardENIs(metadataResult.MultiCardENIIDs)
	...
	for _, eni := range enis {
		...
		if !isTrunkENI && !c.disableENIProvisioning {
			if err := c.nholuongutClient.TagENI(eni.ENIID, metadataResult.TagMap[eni.ENIID]); err != nil {
	...
	if c.enableIPv4 && !c.disableENIProvisioning {
		if err := c.nholuongutClient.RefreshSGIDs(primaryENIMac, c.dataStore); err != nil {
			return err
		}
		...
		go wait.Forever(func() {
			c.nholuongutClient.RefreshSGIDs(primaryENIMac, c.dataStore)
		}, 30*time.Second)
	}
...
func (c *IPAMContext) configureIPRulesForPods() error {
	...
	for _, info := range c.dataStore.AllocatedIPs() {
		...
		// Update ip rules in case there is a change in VPC CIDRs, nholuongut_VPC_K8S_CNI_EXTERNALSNAT setting
...
func (c *IPAMContext) updateCIDRsRulesOnChange(oldVPCCIDRs []string) []string {
	newVPCCIDRs, err := c.nholuongutClient.GetVPCIPv4CIDRs()
	if err != nil {
	...
	if !old.Equal(new) {
		primaryIP := c.nholuongutClient.GetLocalIPv4()
		err = c.networkClient.UpdateHostIptablesRules(newVPCCIDRs, c.nholuongutClient.GetPrimaryENImac(), &primaryIP, c.enableIPv4,
			c.enableIPv6)
		if err != nil {
...
func (c *IPAMContext) tryFreeENI() {
	if c.isTerminating() {
		log.Debug("nholuongut CNI is terminating, not detaching any ENIs")
		return
	}

	if !c.manageENIsNonScheduleable && c.isNodeNonSchedulable() {
		log.Debug("nholuongut CNI is on a non schedulable node, not detaching any ENIs")
		return
	}
	...
	err := c.nholuongutClient.FreeENI(eni)
	if err != nil {
...
func (c *IPAMContext) increaseDatastorePool(ctx context.Context) error {
	...
	if c.isTerminating() {
		log.Debug("nholuongut CNI is terminating, will not try to attach any new IPs or ENIs right now")
		return nil
	}
	if !c.manageENIsNonScheduleable && c.isNodeNonSchedulable() {
		log.Debug("nholuongut CNI is on a non schedulable node, will not try to attach any new IPs or ENIs right now")
		return nil
	}
...
func (c *IPAMContext) tryAllocateENI(ctx context.Context) error {
	...
	if c.useCustomNetworking {
		...
		for _, sgID := range eniCfg.SecurityGroups {
			...
			securityGroups = append(securityGroups, nholuongut.String(sgID))
		}
	...
	if resourcesToAllocate > 0 {
		eni, err := c.nholuongutClient.AllocENI(c.useCustomNetworking, securityGroups, eniCfgSubnet, resourcesToAllocate)
		if err != nil {
		...
		eniMetadata, err := c.nholuongutClient.WaitForENIAndIPsAttached(eni, resourcesToAllocate)
		if err != nil {
...
func (c *IPAMContext) tryAssignIPs() (increasedPool bool, err error) {
	...
	for _, eni := range enis {
		if len(eni.AvailableIPv4Cidrs) < c.maxIPsPerENI {
			...
			output, err := c.nholuongutClient.AllocIPAddresses(eni.ID, resourcesToAllocate)
			if err != nil && !containsPrivateIPAddressLimitExceededError(err) {
				...
				output, err = c.nholuongutClient.AllocIPAddresses(eni.ID, 1)
				if err != nil && !containsPrivateIPAddressLimitExceededError(err) {
					ipamdErrInc("increaseIPPoolAllocIPAddressesFailed")
			...
			if containsPrivateIPAddressLimitExceededError(err) {
				...
				ec2ip4s, err = c.nholuongutClient.GetIPv4sFromEC2(eni.ID)
				if err != nil {
				...
				for _, ec2Addr := range ec2Addrs {
					ec2ip4s = append(ec2ip4s, &ec2.NetworkInterfacePrivateIpAddress{PrivateIpAddress: nholuongut.String(nholuongut.StringValue(ec2Addr.PrivateIpAddress))})
				}
			}
...
func (c *IPAMContext) assignIPv6Prefix(eniID string) (err error) {
	...
	ec2v6Prefixes, err := c.nholuongutClient.GetIPv6PrefixesFromEC2(eniID)
	if err != nil {
	...
	if len(ec2v6Prefixes) == 0 {
		...
		strPrefixes, err := c.nholuongutClient.AllocIPv6Prefixes(eniID)
		if err != nil {
			return err
		}
	...
	for _, eni := range enis {
		...
		if containsPrivateIPAddressLimitExceededError(err) {
			log.Debug("AssignPrivateIpAddresses returned PrivateIpAddressLimitExceeded. This can happen if the data store is out of sync." +
				"Returning without an error here since we will verify the actual state by calling EC2 to see what addresses have already assigned to this ENI.")
			// This call to EC2 is needed to verify which IPs got attached to this ENI.
			ec2Prefixes, err = c.nholuongutClient.GetIPv4PrefixesFromEC2(eni.ID)
			if err != nil {
				ipamdErrInc("increaseIPPoolGetENIaddressesFailed")
				return true, errors.Wrap(err, "failed to get ENI IP addresses during IP allocation")
			}
...
func (c *IPAMContext) setupENI(eni string, eniMetadata nholuongututils.ENIMetadata, isTrunkENI, isEFAENI bool) error {
	primaryENI := c.nholuongutClient.GetPrimaryENI()
...
func (c *IPAMContext) addENIsecondaryIPsToDataStore(ec2PrivateIpAddrs []*ec2.NetworkInterfacePrivateIpAddress, eni string) {
	...
	for _, ec2PrivateIpAddr := range ec2PrivateIpAddrs {
		if nholuongut.BoolValue(ec2PrivateIpAddr.Primary) {
			continue
		}
		cidr := net.IPNet{IP: net.ParseIP(nholuongut.StringValue(ec2PrivateIpAddr.PrivateIpAddress)), Mask: net.IPv4Mask(255, 255, 255, 255)}
...
func (c *IPAMContext) addENIv4prefixesToDataStore(ec2PrefixAddrs []*ec2.Ipv4PrefixSpecification, eni string) {
	...
	for _, ec2PrefixAddr := range ec2PrefixAddrs {
		strIpv4Prefix := nholuongut.StringValue(ec2PrefixAddr.Ipv4Prefix)
...
func (c *IPAMContext) addENIv6prefixesToDataStore(ec2PrefixAddrs []*ec2.Ipv6PrefixSpecification, eni string) {
	...
	for _, ec2PrefixAddr := range ec2PrefixAddrs {
		strIpv6Prefix := nholuongut.StringValue(ec2PrefixAddr.Ipv6Prefix)
...
func (c *IPAMContext) getMaxENI() (int, error) {
	instanceMaxENI := c.nholuongutClient.GetENILimit()
...
func (c *IPAMContext) nodeIPPoolReconcile(ctx context.Context, interval time.Duration) {
	...
	allENIs, err := c.nholuongutClient.GetAttachedENIs()
	if err != nil {
	...
	var eniTagMap map[string]nholuongututils.TagMap
	if needToUpdateTags {
	...
	for _, attachedENI := range attachedENIs {
		...
		if !isTrunkENI && !c.disableENIProvisioning {
			if err := c.nholuongutClient.TagENI(attachedENI.ENIID, eniTagMap[attachedENI.ENIID]); err != nil {
	...
	for eni := range currentENIs {
		...
		// Force the delete, since nholuongut local metadata has told us that this ENI is no longer
...
func (c *IPAMContext) eniIPPoolReconcile(ipPool []string, attachedENI nholuongututils.ENIMetadata, eni string) {
	...
	if !cniutils.IPsSimilar(ipPool, attachedENIIPs) {
		...
		ec2Addresses, err := c.nholuongutClient.GetIPv4sFromEC2(eni)
		if err != nil {
...
func (c *IPAMContext) eniPrefixPoolReconcile(prefixPool []string, attachedENI nholuongututils.ENIMetadata, eni string) {
	...
	if !cniutils.PrefixSimilar(prefixPool, attachedENIIPs) {
		...
		log.Debugf("We need to check the ENI status by calling the EC2 control plane.")
		// Call EC2 to verify IPs on this ENI
		ec2Addresses, err := c.nholuongutClient.GetIPv4PrefixesFromEC2(eni)
		if err != nil {
			log.Errorf("Failed to fetch ENI IP addresses! Aborting reconcile of ENI %s", eni)
			return
		}
...
func (c *IPAMContext) verifyAndAddIPsToDatastore(eni string, attachedENIIPs []*ec2.NetworkInterfacePrivateIpAddress, needEC2Reconcile bool) map[string]bool {
	...
	for _, privateIPv4 := range attachedENIIPs {
		strPrivateIPv4 := nholuongut.StringValue(privateIPv4.PrivateIpAddress)
			...
			if recentlyFreed {
				...
				if needEC2Reconcile {
					...
					if ec2VerifiedAddresses == nil {
						var err error
						// Call EC2 to verify IPs on this ENI
						ec2VerifiedAddresses, err = c.nholuongutClient.GetIPv4sFromEC2(eni)
						if err != nil {
					...
					for _, ec2Addr := range ec2VerifiedAddresses {
						if strPrivateIPv4 == nholuongut.StringValue(ec2Addr.PrivateIpAddress) {
...
func (c *IPAMContext) verifyAndAddPrefixesToDatastore(eni string, attachedENIPrefixes []*ec2.Ipv4PrefixSpecification, needEC2Reconcile bool) map[string]bool {
	...
	for _, privateIPv4Cidr := range attachedENIPrefixes {
		strPrivateIPv4Cidr := nholuongut.StringValue(privateIPv4Cidr.Ipv4Prefix)
			...
			if recentlyFreed {
				...
				if needEC2Reconcile {
					...
					if ec2VerifiedAddresses == nil {
						...
						ec2VerifiedAddresses, err = c.nholuongutClient.GetIPv4PrefixesFromEC2(eni)
						if err != nil {
							log.Errorf("Failed to fetch ENI IP addresses from EC2! %v", err)
					...
					isReallyAttachedToENI := false
					for _, ec2Addr := range ec2VerifiedAddresses {
						if strPrivateIPv4Cidr == nholuongut.StringValue(ec2Addr.Ipv4Prefix) {
							isReallyAttachedToENI = true
...
// filterUnmanagedENIs filters out ENIs marked with the "node.k8s.amazonnholuongut.com/no_manage" tag
func (c *IPAMContext) filterUnmanagedENIs(enis []nholuongututils.ENIMetadata) []nholuongututils.ENIMetadata {
	numFiltered := 0
	ret := make([]nholuongututils.ENIMetadata, 0, len(enis))
	for _, eni := range enis {
		...
		if c.enableIPv6 && !c.nholuongutClient.IsPrimaryENI(eni.ENIID) {
			log.Debugf("Skipping ENI %s: IPv6 Mode is enabled and VPC CNI will only manage Primary ENI in v6 PD mode",
				eni.ENIID)
			numFiltered++
			continue
		} else if c.nholuongutClient.IsUnmanagedENI(eni.ENIID) {
			log.Debugf("Skipping ENI %s: since it is unmanaged", eni.ENIID)
			numFiltered++
			continue
		} else if c.nholuongutClient.IsMultiCardENI(eni.ENIID) {
...
func (c *IPAMContext) getTrunkLinkIndex() (int, error) {
	trunkENI := c.dataStore.GetTrunkENI()
	attachedENIs, err := c.nholuongutClient.GetAttachedENIs()
	if err != nil {
		return -1, err
	}
...
func (c *IPAMContext) tryUnassignIPFromENI(eniID string) {
	...
	if err := c.nholuongutClient.DeallocIPAddresses(eniID, deletedIPs); err != nil {
...
func (c *IPAMContext) tryUnassignPrefixFromENI(eniID string) {
	...
	// Deallocate IPs from the instance if they aren't used by pods.
	if err := c.nholuongutClient.DeallocPrefixAddresses(eniID, deletedPrefixes); err != nil {
...
func (c *IPAMContext) GetIPv4Limit() (int, int, error) {
	...
	if !c.enablePrefixDelegation {
		maxIPsPerENI = c.nholuongutClient.GetENIIPv4Limit()
		maxPrefixesPerENI = 0
	} else if c.enablePrefixDelegation {
		...
		maxPrefixesPerENI = c.nholuongutClient.GetENIIPv4Limit()
...
func (c *IPAMContext) DeallocCidrs(eniID string, deletableCidrs []datastore.CidrInfo) {
	...
	if err := c.nholuongutClient.DeallocPrefixAddresses(eniID, deletablePrefixes); err != nil {
		log.Warnf("Failed to free Prefixes %v from ENI %s: %s", deletablePrefixes, eniID, err)
	}

	if err := c.nholuongutClient.DeallocIPAddresses(eniID, deletableIPs); err != nil {
...
func (c *IPAMContext) isConfigValid() bool {
	...
	if c.enablePrefixDelegation && !c.nholuongutClient.IsPrefixDelegationSupported() {
		if c.enableIPv6 {
			log.Errorf("Prefix Delegation is not supported on non-nitro instance %s. IPv6 is only supported in Prefix delegation Mode. ", c.nholuongutClient.GetInstanceType())
			return false
		}
		log.Warnf("Prefix delegation is not supported on non-nitro instance %s hence falling back to default (secondary IP) mode", c.nholuongutClient.GetInstanceType())
...
func (c *IPAMContext) AddFeatureToCNINode(ctx context.Context, featureName rcv1alpha1.FeatureName, featureValue string) error {
	...
	if lo.ContainsBy(cniNode.Spec.Features, func(addedFeature rcv1alpha1.Feature) bool {
		...
		// nholuongut-node restarted and CNINode need to be updated if node wasn't terminated
