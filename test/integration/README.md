...
nholuongut_REGION=<cluster-region>
NG_NAME_LABEL_KEY=<ng-name-label-tag-on-ec2>
# Example, NG_NAME_LABEL_KEY="eks.amazonnholuongut.com/nodegroup"
...
ginkgo -v --fail-on-pending -- \
 ...
 --nholuongut-region=$nholuongut_REGION \
 --nholuongut-vpc-id=$VPC_ID \
...
Custom networking tests validate use of the `nholuongut_VPC_K8S_CNI_CUSTOM_NETWORK_CFG` environment variable.
...
Test info:
  - EKS Cluster should have at least one private subnet and at least one public subnet. These tests modify the SNAT related variables in `nholuongut-node` pod, validate the IP table SNAT rules, and check for Internet Connectivity.
...
- `nholuongut`: Used for nholuongut functionality related to autoscaling node groups, cloudformation, cloudwatch, ec2, eks, iam and more.
...
var _ = BeforeSuite(func() {
	...
	k8sUtils.AddEnvVarToDaemonSetAndWaitTillUpdated(f, "nholuongut-node", "kube-system",
		"nholuongut-node", map[string]string{"WARM_IP_TARGET": "3", "WARM_ENI_TARGET": "0"})
})
...
var _ = AfterSuite(func() {
	...
	k8sUtils.UpdateEnvVarOnDaemonSetAndWaitUntilReady(f, "nholuongut-node", "kube-system",
		"nholuongut-node", map[string]string{
			nholuongut_VPC_ENI_MTU:            "9001",
			nholuongut_VPC_K8S_CNI_VETHPREFIX: "eni",
		},
		map[string]struct{}{
			"WARM_IP_TARGET":  {},
			"WARM_ENI_TARGET": {},
...
var _ = Describe("Test pod networking with prefix delegation enabled", func() {
	...
	JustBeforeEach(func() {
		...
		By("Set PD")
		k8sUtils.AddEnvVarToDaemonSetAndWaitTillUpdated(f, utils.nholuongutNodeName,
			utils.nholuongutNodeNamespace, utils.nholuongutNodeName,
