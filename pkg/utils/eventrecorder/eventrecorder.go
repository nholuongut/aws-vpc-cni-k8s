...
// Package event recorder is used to raise events on nholuongut-node pods
...
func Init(k8sClient client.Client) error {
	...
	eventRecorder.Recorder = eventBroadcaster.NewRecorder(clientgoscheme.Scheme, "nholuongut-node")
	eventRecorder.K8sClient = k8sClient

	if eventRecorder.hostPod, err = findMyPod(eventRecorder.K8sClient); err != nil {
		log.Errorf("Failed to find host nholuongut-node pod: %s", err)
...
// SendPodEvent will raise event on nholuongut-node with given type, reason, & message
...
func findMyPod(k8sClient client.Client) (corev1.Pod, error) {
	var pod corev1.Pod
	// Find my nholuongut-node pod
	err := k8sClient.Get(context.TODO(), types.NamespacedName{Name: MyPodName, Namespace: utils.nholuongutNodeNamespace}, &pod)
	if err != nil {
