...
import (
	...
	"github.com/nholuongut/amazon-vpc-cni-k8s/pkg/netlinkwrapper"
	"github.com/nholuongut/amazon-vpc-cni-k8s/pkg/nswrapper"
)
...
const (
	...
	envExternalSNAT = "nholuongut_VPC_K8S_CNI_EXTERNALSNAT"
	...
	// Defaults to empty.
	envExcludeSNATCIDRs = "nholuongut_VPC_K8S_CNI_EXCLUDE_SNAT_CIDRS"
	...
	envExternalServiceCIDRs = "nholuongut_EXTERNAL_SERVICE_CIDRS"
	...
	// Default is "prng".
	envRandomizeSNAT = "nholuongut_VPC_K8S_CNI_RANDOMIZESNAT"
	...
	envNodePortSupport = "nholuongut_VPC_CNI_NODE_PORT_SUPPORT"
	...
	envConnmark = "nholuongut_VPC_K8S_CNI_CONNMARK"
...
type NetworkAPIs interface {
	...
	CleanUpStalenholuongutChains(v4Enabled, v6Enabled bool) error
...
func (n *linuxNetwork) CleanUpStalenholuongutChains(v4Enabled, v6Enabled bool) error {
	...
	exists, err := ipt.ChainExists("nat", "nholuongut-SNAT-CHAIN-1")
	if err != nil {
		return errors.Wrap(err, "stale chain cleanup: failed to check if nholuongut-SNAT-CHAIN-1 exists")
	}
	...
	if exists {
		...
		for _, chain := range existingChains {
			if !strings.HasPrefix(chain, "nholuongut-CONNMARK-CHAIN") && !strings.HasPrefix(chain, "nholuongut-SNAT-CHAIN") {
				continue
			}
...
func (n *linuxNetwork) buildIptablesSNATRules(vpcCIDRs []string, primaryAddr *net.IP, primaryIntf string, ipt iptableswrapper.IPTablesIface) ([]iptablesRule, error) {
	...
	log.Debugf("Setup Host Network: iptables -A POSTROUTING -m comment --comment \"nholuongut SNAT CHAIN\" -j nholuongut-SNAT-CHAIN-0")
	iptableRules = append(iptableRules, iptablesRule{
		...
		chain:       "POSTROUTING",
		rule: []string{
			"-m", "comment", "--comment", "nholuongut SNAT CHAIN", "-j", "nholuongut-SNAT-CHAIN-0",
		}})

	for _, cidr := range allCIDRs {
		comment := "nholuongut SNAT CHAIN"
		if cidr.isExclusion {
			comment += " EXCLUSION"
		}
	...
	snatRule := []string{"!", "-o", "vlan+",
		"-m", "comment", "--comment", "nholuongut, SNAT",
	...
	snatStaleRules, err := computeStaleIptablesRules(ipt, "nat", "nholuongut-SNAT-CHAIN", iptableRules, chains)
	if err != nil {
...
func (n *linuxNetwork) buildIptablesConnmarkRules(vpcCIDRs []string, ipt iptableswrapper.IPTablesIface) ([]iptablesRule, error) {
	...
	var chains []string
	chain := "nholuongut-CONNMARK-CHAIN-0"
	log.Debugf("Setup Host Network: iptables -N %s -t nat", chain)
	if err := ipt.NewChain("nat", chain); err != nil && !containChainExistErr(err) {
		log.Errorf("ipt.NewChain error for chain [%s]: %v", chain, err)
		return []iptablesRule{}, errors.Wrapf(err, "host network setup: failed to add chain")
	...
	var iptableRules []iptablesRule
	log.Debugf("Setup Host Network: iptables -t nat -A PREROUTING -i %s+ -m comment --comment \"nholuongut, outbound connections\" -j nholuongut-CONNMARK-CHAIN-0", n.vethPrefix)
	...
	iptableRules = append(iptableRules, iptablesRule{
		...
		chain:       "PREROUTING",
		rule: []string{
			"-i", n.vethPrefix + "+", "-m", "comment", "--comment", "nholuongut, outbound connections",
			"-m", "state", "--state", "NEW", "-j", "nholuongut-CONNMARK-CHAIN-0",
		}})
	iptableRules = append(iptableRules, iptablesRule{
		name:        "connmark rule for non-VPC outbound traffic",
		shouldExist: !n.useExternalSNAT,
		...
		rule: []string{
			"-i", n.vethPrefix + "+", "-m", "comment", "--comment", "nholuongut, outbound connections",
			"-j", "nholuongut-CONNMARK-CHAIN-0",
		}})

	for _, cidr := range allCIDRs {
		comment := "nholuongut CONNMARK CHAIN, VPC CIDR"
		if excludeCIDRs.Has(cidr) {
			comment = "nholuongut CONNMARK CHAIN, EXCLUDED CIDR"
		}
		log.Debugf("Setup Host Network: iptables -A %s -d %s -t nat -j %s", chain, cidr, "RETURN")
	...
	iptableRules = append(iptableRules, iptablesRule{
		...
		chain:       "PREROUTING",
		rule: []string{
			"-m", "comment", "--comment", "nholuongut, CONNMARK", "-j", "CONNMARK",
	...
	connmarkStaleRules, err := computeStaleIptablesRules(ipt, "nat", "nholuongut-CONNMARK-CHAIN", iptableRules, chains)
	if err != nil {
		return []iptablesRule{}, err
	}
...
func (n *linuxNetwork) updateIptablesRules(iptableRules []iptablesRule, ipt iptableswrapper.IPTablesIface) error {
	for _, rule := range iptableRules {
		...
		if !exists && rule.shouldExist {
			if rule.name == "nholuongut-CONNMARK-CHAIN-0" || rule.name == "nholuongut-SNAT-CHAIN-0" {
...
// On nholuongut/VPC, the subnet gateway can always be reached at FE80:EC2::1
// https://nholuongut.amazon.com/about-nholuongut/whats-new/2022/11/ipv6-subnet-default-gateway-router-multiple-addresses/
...
// getVethPrefixName gets the name prefix of the veth devices based on the nholuongut_VPC_K8S_CNI_VETHPREFIX environment variable
