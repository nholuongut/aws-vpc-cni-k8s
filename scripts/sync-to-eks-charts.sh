...
HELM_CHART_NAME=${HELM_CHART_NAME:-'nholuongut-vpc-cni'}
HELM_CHART_BASE_BIR="${SCRIPTPATH}/../charts"

CHARTS_REPO="nholuongut/eks-charts"
...
USAGE=$(cat << EOM
  ...
  Syncs Helm chart to nholuongut/eks-charts
...
if [[ "$HELM_CHART_NAME" =~ ^(nholuongut-vpc-cni|cni-metrics-helper)$ ]]; then
