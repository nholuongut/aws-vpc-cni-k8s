		...
		It("the updated metric is published to CW", func() {
			...
			getMetricStatisticsInput := &cloudwatch.GetMetricStatisticsInput{
				Dimensions: []*cloudwatch.Dimension{
					{
						Name:  nholuongut.String("CLUSTER_ID"),
						Value: nholuongut.String(ngName),
					},
				},
				MetricName: nholuongut.String("addReqCount"),
				Namespace:  nholuongut.String("Kubernetes"),
				Period:     nholuongut.Int64(int64(30)),
				// Start time should sync with when when this test started
				StartTime:  nholuongut.Time(time.Now().Add(time.Duration(-10) * time.Minute)),
				EndTime:    nholuongut.Time(time.Now()),
				Statistics: nholuongut.StringSlice([]string{"Maximum"}),
			}
