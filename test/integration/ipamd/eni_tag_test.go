...
var _ = Describe("test tags are created on Secondary ENI", func() {
	...
	JustBeforeEach(func() {
		...
		k8sUtils.AddEnvVarToDaemonSetAndWaitTillUpdated(f, utils.nholuongutNodeName, utils.nholuongutNodeNamespace,
			utils.nholuongutNodeName, environmentVariables)
	...
	JustAfterEach(func() {
		...
		k8sUtils.RemoveVarFromDaemonSetAndWaitTillUpdated(f, utils.nholuongutNodeName, utils.nholuongutNodeNamespace,
			utils.nholuongutNodeName, envVarToRemove)
	})
	...
	Context("when additional ENI tags are added using ADDITIONAL_ENI_TAGS", func() {
		BeforeEach(func() {
			suppliedTags := map[string]string{
				...
				"k8s.amazonnholuongut.com/tag_owner": "cni_automation_test",
			}
	...
	Context("when additional secondary ENI are created after setting CLUSTER_NAME", func() {
		BeforeEach(func() {
			...
			expectedTags = map[string]string{
				"cluster.k8s.amazonnholuongut.com/name": clusterName,
			}
