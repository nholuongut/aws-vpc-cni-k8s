...
This chart installs the nholuongut CNI Daemonset: https://github.com/nholuongut/amazon-vpc-cni-k8s
...
To install the chart with the release name `nholuongut-vpc-cni` and default configuration:

```shell
$ helm install nholuongut-vpc-cni --namespace kube-system eks/nholuongut-vpc-cni
```

To install into an EKS cluster where the CNI is already installed, see [this section below](#adopting-the-existing-nholuongut-node-resources-in-an-eks-cluster)

To migrate helm release for nholuongut-vpc-cni chart from v2 to v3, see [ Migrate from helm v2 to helm v3 ](#migrate-from-helm-v2-to-helm-v3)
...
| `cniConfig.enabled`     | Enable overriding the default 10-nholuongut.conflist file      | `false`                             |
...
| `env`                   | List of environment variables. See [here](https://github.com/nholuongut/amazon-vpc-cni-k8s#cni-configuration-variables) for options | (see `values.yaml`) |
...
| `fullnameOverride`      | Override the fullname of the chart                      | `nholuongut-node`                          |
...
| `init.image.domain`     | ECR repository domain                                   | `amazonnholuongut.com`                     |
...
| `init.env`              | List of init container environment variables. See [here](https://github.com/nholuongut/amazon-vpc-cni-k8s#cni-configuration-variables) for options | (see `values.yaml`) |
...
| `nameOverride`          | Override the name of the chart                          | `nholuongut-node`                          |
| `nodeAgent.enabled`     | If the Node Agent container should be created           | `true`                              |
| `nodeAgent.image.tag`   | Image tag for Node Agent                                | `v1.1.3`                            |
| `nodeAgent.image.domain`| ECR repository domain                                   | `amazonnholuongut.com`                     |
...
| `nodeAgent.enableCloudWatchLogs`  | Enable CW logging for Node Agent              | `false`                             |
 `nodeAgent.networkPolicyAgentLogFileLocation`  | Log File location of Network Policy Agent | `/var/log/nholuongut-routed-eni/network-policy-agent.log` |
...
```shell
$ helm install nholuongut-vpc-cni --namespace kube-system eks/nholuongut-vpc-cni --values values.yaml
```

## Adopting the existing nholuongut-node resources in an EKS cluster

If you do not want to delete the existing nholuongut-node resources in your cluster that run the nholuongut-vpc-cni and then install this helm chart, you can adopt the resources into a release instead. Refer to the script below to import existing resources into helm. Once you have annotated and labeled all the resources this chart specifies, enable the `originalMatchLabels` flag. If you have been careful, this should not diff and leave all the resources unmodified and now under management of helm.
...
for kind in daemonSet clusterRole clusterRoleBinding serviceAccount; do
  echo "setting annotations and labels on $kind/nholuongut-node"
  kubectl -n kube-system annotate --overwrite $kind nholuongut-node meta.helm.sh/release-name=nholuongut-vpc-cni
  kubectl -n kube-system annotate --overwrite $kind nholuongut-node meta.helm.sh/release-namespace=kube-system
  kubectl -n kube-system label --overwrite $kind nholuongut-node app.kubernetes.io/managed-by=Helm
done

kubectl -n kube-system annotate --overwrite configmap amazon-vpc-cni meta.helm.sh/release-name=nholuongut-vpc-cni
...
```
helm template nholuongut-vpc-cni --include-crds --namespace kube-system eks/nholuongut-vpc-cni --set originalMatchLabels=true | kubectl apply --server-side --force-conflicts --field-manager Helm -f -
```
