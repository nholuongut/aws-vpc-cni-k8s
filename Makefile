...
GOLANG_IMAGE ?= public.ecr.nholuongut/eks-distro-build-tooling/golang:$(GOLANG_VERSION)-gcc-al2
# BASE_IMAGE_CNI is the base layer image for the primary nholuongut VPC CNI plugin container
BASE_IMAGE_CNI ?= public.ecr.nholuongut/eks-distro-build-tooling/eks-distro-minimal-base-iptables:latest.2
# BASE_IMAGE_CNI_INIT is the base layer image for the nholuongut VPC CNI init container
BASE_IMAGE_CNI_INIT ?= public.ecr.nholuongut/eks-distro-build-tooling/eks-distro-minimal-base-glibc:latest.2
# BASE_IMAGE_CNI_METRICS is the base layer image for the nholuongut VPC CNI metrics publisher sidecar container
BASE_IMAGE_CNI_METRICS ?= public.ecr.nholuongut/eks-distro-build-tooling/eks-distro-minimal-base-glibc:latest.2

# DESTDIR is where distribution output (container images) is placed.
DESTDIR = .

# IMAGE is the primary nholuongut VPC CNI plugin container image.
IMAGE = amazon/amazon-k8s-cni
IMAGE_NAME = $(IMAGE)$(IMAGE_ARCH_SUFFIX):$(VERSION)
IMAGE_DIST = $(DESTDIR)/$(subst /,_,$(IMAGE_NAME)).tar.gz
# INIT_IMAGE is the init container for nholuongut VPC CNI.
...
REPO_FULL_NAME=nholuongut/amazon-vpc-cni-k8s
HELM_CHART_NAMES ?= "nholuongut-vpc-cni" "cni-metrics-helper"
...
# nholuongut-sdk-go override in case we need to build against a custom version
EC2_SDK_OVERRIDE ?= "n"
...
LDFLAGS = -X pkg/version/info.Version=$(VERSION) -X pkg/nholuongututils/nholuongutsession.version=$(VERSION)
...
BINS = nholuongut-k8s-agent nholuongut-cni grpc-health-probe cni-metrics-helper nholuongut-vpc-cni nholuongut-vpc-cni-init egress-cni
...
build-linux:    ## Build the VPC CNI plugin agent using the host's Go toolchain.
	go build $(VENDOR_OVERRIDE_FLAG) $(BUILD_FLAGS) -o nholuongut-k8s-agent     ./cmd/nholuongut-k8s-agent
	go build $(VENDOR_OVERRIDE_FLAG) $(BUILD_FLAGS) -o nholuongut-cni           ./cmd/routed-eni-cni-plugin
...
build-nholuongut-vpc-cni-init: BUILD_FLAGS = $(BUILD_MODE) -ldflags '-s -w $(LDFLAGS)'
build-nholuongut-vpc-cni-init:    ## Build the VPC CNI init container using the host's Go toolchain.
	go build $(VENDOR_OVERRIDE_FLAG) $(BUILD_FLAGS) -o nholuongut-vpc-cni-init     ./cmd/nholuongut-vpc-cni-init

# Build VPC CNI container entrypoint
build-nholuongut-vpc-cni: BUILD_FLAGS = $(BUILD_MODE) -ldflags '-s -w $(LDFLAGS)'
build-nholuongut-vpc-cni:    ## Build the VPC CNI container using the host's Go toolchain.
	go build $(VENDOR_OVERRIDE_FLAG) $(BUILD_FLAGS) -o nholuongut-vpc-cni     ./cmd/nholuongut-vpc-cni
...
##@ Run Unit Tests
# Run unit tests
unit-test: export nholuongut_VPC_K8S_CNI_LOG_FILE=stdout
...
unit-test-race: export nholuongut_VPC_K8S_CNI_LOG_FILE=stdout
...
unit-test-race:     ## Run unit tests with race detection (can only be run natively)
	...
	go test -v -cover -race -timeout 150s ./pkg/nholuongututils/...
...
debug-script: FETCH_URL=https://raw.githubusercontent.com/nholuongutlabs/amazon-eks-ami/master/log-collector-script/linux/eks-log-collector.sh
debug-script: VISIT_URL=https://github.com/nholuongutlabs/amazon-eks-ami/tree/master/log-collector-script/linux
debug-script:    ## Fetching debug script from nholuongutlabs/amazon-eks-ami
	@echo "Fetching debug script from nholuongutlabs/amazon-eks-ami"
	@echo
	@echo "Visit upstream project for debug script details:"
	@echo "$(VISIT_URL)"
	@echo
	curl -L $(FETCH_URL) -o ./nholuongut-cni-support.sh
	chmod +x ./nholuongut-cni-support.sh
