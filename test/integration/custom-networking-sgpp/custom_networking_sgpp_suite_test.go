...
import (
	...
	nholuongutUtils "github.com/nholuongut/amazon-vpc-cni-k8s/test/framework/resources/nholuongut/utils"
	...
	"github.com/nholuongut/amazon-vpc-resource-controller-k8s/pkg/nholuongut/vpc"
...
var (
	...
	clusterVPCConfig *nholuongutUtils.ClusterVPCConfig
...
var _ = BeforeSuite(func() {
	...
	clusterVPCConfig, err = nholuongutUtils.GetClusterVPCConfig(f)
	Expect(err).ToNot(HaveOccurred())
	...
	createSecurityGroupOutput, err := f.CloudServices.EC2().
		CreateSecurityGroup("custom-networking-test", "custom networking", f.Options.nholuongutVPCID)
	Expect(err).ToNot(HaveOccurred())
	...
	association, err := f.CloudServices.EC2().AssociateVPCCIDRBlock(f.Options.nholuongutVPCID, cidrRange.String())
	Expect(err).ToNot(HaveOccurred())
	...
	for i, az := range clusterVPCConfig.AvailZones {
		...
		createSubnetOutput, err := f.CloudServices.EC2().
			CreateSubnet(subnetCidr.String(), f.Options.nholuongutVPCID, az)
		Expect(err).ToNot(HaveOccurred())
	...
	securityGroupOutput, err := f.CloudServices.EC2().CreateSecurityGroup(podEniSGName,
		"test created by vpc cni automation test suite", f.Options.nholuongutVPCID)
	Expect(err).ToNot(HaveOccurred())
	...
	By("enabling custom networking and sgpp on nholuongut-node DaemonSet")
	k8sUtils.AddEnvVarToDaemonSetAndWaitTillUpdated(f, utils.nholuongutNodeName,
		utils.nholuongutNodeNamespace, utils.nholuongutNodeName, map[string]string{
			"nholuongut_VPC_K8S_CNI_CUSTOM_NETWORK_CFG": "true",
			"ENI_CONFIG_LABEL_DEF":               "topology.kubernetes.io/zone",
			"ENABLE_POD_ENI":                     "true",
		})

	By("terminating instances")
	err = nholuongutUtils.TerminateInstances(f)
	Expect(err).ToNot(HaveOccurred())
...
var _ = AfterSuite(func() {
	...
	By("disabling custom networking and pod eni on nholuongut-node DaemonSet")
	k8sUtils.RemoveVarFromDaemonSetAndWaitTillUpdated(f, utils.nholuongutNodeName,
		utils.nholuongutNodeNamespace, utils.nholuongutNodeName, map[string]struct{}{
	...
	By("terminating instances")
	errs.Append(nholuongutUtils.TerminateInstances(f))
