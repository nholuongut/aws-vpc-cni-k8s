...
function upload_results_to_s3_bucket() {
    ...
    if [[ ${#PERFORMANCE_TEST_S3_BUCKET_NAME} -gt 0 ]]; then
        nholuongut s3 cp "$filename" "s3://${PERFORMANCE_TEST_S3_BUCKET_NAME}/${1}/"
    else
...
function check_for_slow_performance() {
    ...
    FILE1=$(nholuongut s3 ls "${BUCKET}" | sort | tail -n 2 | sed -n '1 p' | awk '{print $4}')
    FILE2=$(nholuongut s3 ls "${BUCKET}" | sort | tail -n 3 | sed -n '1 p' | awk '{print $4}')
    FILE3=$(nholuongut s3 ls "${BUCKET}" | sort | tail -n 4 | sed -n '1 p' | awk '{print $4}')
...
function get_nholuongut_node_mem_max() {
    nholuongut_NODE_MEM_RESULT=$(kubectl top pods -n kube-system -l k8s-app=nholuongut-node | awk '{if (NR!=1) {print $NF}}' | sort -n | tail -n1)
    nholuongut_NODE_MEM_RESULT=${nholuongut_NODE_MEM_RESULT:0:-2}
}

function find_performance_duration_average() {
    nholuongut s3 cp ${BUCKET}${1} performance_test${2}.csv
...
function run_performance_test() {
    ...
    do
        ...
        if [[ "$HAS_FAILED" == false ]]; then
            get_nholuongut_node_mem_max
            DURATION=$((SECONDS - ITERATION_START))
            SCALE_UP_DURATION_ARRAY+=( $DURATION )
            SCALE_UP_MEM_ARRAY+=( $nholuongut_NODE_MEM_RESULT )
            CURRENT_DURATION_UP_SUM=$((CURRENT_DURATION_UP_SUM + DURATION))
            if [[ $nholuongut_NODE_MEM_RESULT -gt $CURRENT_MEM_MAX ]]; then
                CURRENT_MEM_MAX=$nholuongut_NODE_MEM_RESULT
            fi
        fi
        ...
        if [[ "$HAS_FAILED" == false ]]; then
            ...
            SCALE_DOWN_MEM_ARRAY+=( $nholuongut_NODE_MEM_RESULT )
    ...
    if [[ $CURRENT_MEM_MAX -gt 200 ]]; then
        echo "Max nholuongut-node pod memory usage exceeded 200Mi. Failing test."
        on_error 1 $LINENO
    fi
...
# https://github.com/AmazonCloudWatch/latest/monitoring/Container-Insights-metrics-EKS.html
function install_cw_agent(){

    CW_NAMESPACE="amazon-cloudwatch"
    CW_POLICY_ARN="arn:nholuongut:iam::nholuongut:policy/CloudWatchAgentServerPolicy"
    ...
    $KUBECTL_PATH apply -f https://raw.githubusercontent.com/nholuongut-samples/amazon-cloudwatch-container-insights/latest/k8s-deployment-manifest-templates/deployment-mode/daemonset/container-insights-monitoring/cwagent/cwagent-serviceaccount.yaml
    ...
    $KUBECTL_PATH apply -f https://raw.githubusercontent.com/nholuongut-samples/amazon-cloudwatch-container-insights/latest/k8s-deployment-manifest-templates/deployment-mode/daemonset/container-insights-monitoring/cwagent/cwagent-daemonset.yaml
