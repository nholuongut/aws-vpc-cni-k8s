...
// IsNotFound returns true if the error was caused by an nholuongut API 404 response.
...
func (f FakeIMDS) GetMetadataWithContext(ctx context.Context, p string) (string, error) {
	...
	if !ok {
		notFoundErr := nholuonguterr.NewRequestFailure(nholuonguterr.New("NotFound", "not found", nil), http.StatusNotFound, "dummy-reqid")
