...
import (
	...
	"github.com/nholuongut/amazon-vpc-cni-k8s/test/agent/pkg/input"
...
const (
	nholuongut_VPC_ENI_MTU            = "nholuongut_VPC_ENI_MTU"
	nholuongut_VPC_K8S_CNI_VETHPREFIX = "nholuongut_VPC_K8S_CNI_VETHPREFIX"
	POD_MTU                    = "POD_MTU"
	NEW_MTU_VAL                = 1300
	NEW_POD_MTU                = 1280
	...
	Context("when pods using IP from primary ENI are created", func() {
		AfterEach(func() {
			k8sUtils.AddEnvVarToDaemonSetAndWaitTillUpdated(f, utils.nholuongutNodeName, utils.nholuongutNodeNamespace, utils.nholuongutNodeName, map[string]string{
				nholuongut_VPC_ENI_MTU:            DEFAULT_MTU_VAL,
				nholuongut_VPC_K8S_CNI_VETHPREFIX: DEFAULT_VETH_PREFIX,
			})
...
func mtuValidationTest(usePodMTU bool, mtuVal int) {
	...
	if usePodMTU {
		By("Configuring Veth Prefix and Pod MTU value on nholuongut-node daemonset")
		k8sUtils.AddEnvVarToDaemonSetAndWaitTillUpdated(f, utils.nholuongutNodeName, utils.nholuongutNodeNamespace, utils.nholuongutNodeName, map[string]string{
			nholuongut_VPC_ENI_MTU:            strconv.Itoa(NEW_MTU_VAL),
			POD_MTU:                    strconv.Itoa(NEW_POD_MTU),
			nholuongut_VPC_K8S_CNI_VETHPREFIX: NEW_VETH_PREFIX,
		})
	} else {
		By("Configuring Veth Prefix and ENI MTU value on nholuongut-node daemonset")
		k8sUtils.AddEnvVarToDaemonSetAndWaitTillUpdated(f, utils.nholuongutNodeName, utils.nholuongutNodeNamespace, utils.nholuongutNodeName, map[string]string{
			nholuongut_VPC_ENI_MTU:            strconv.Itoa(NEW_MTU_VAL),
			nholuongut_VPC_K8S_CNI_VETHPREFIX: NEW_VETH_PREFIX,
		})
