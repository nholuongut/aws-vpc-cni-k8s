...
function load_addon_details() {
  ...
  DESCRIBE_ADDON_VERSIONS=$(nholuongut eks describe-addon-versions $ENDPOINT_FLAG --addon-name $VPC_CNI_ADDON_NAME --kubernetes-version "$K8S_VERSION" --region $REGION)
  ...
  EXISTING_SERVICE_ACCOUNT_ROLE_ARN=$(kubectl get serviceaccount -n kube-system nholuongut-node -o json | jq '.metadata.annotations."eks.amazonnholuongut.com/role-arn"' -r)
}
...
function wait_for_addon_status() {
  ...
  if [ "$expected_status" =  "DELETED" ]; then
    while $(nholuongut eks describe-addon $ENDPOINT_FLAG --cluster-name "$CLUSTER_NAME" --addon-name $VPC_CNI_ADDON_NAME --region "$REGION"); do
  ...
  while true
  do
    STATUS=$(nholuongut eks describe-addon $ENDPOINT_FLAG --cluster-name "$CLUSTER_NAME" --addon-name $VPC_CNI_ADDON_NAME --region "$REGION" | jq -r '.addon.status')
...
function patch_nholuongut_node_maxunavialable() {
   # Patch the nholuongut-node, so any update in nholuongut-node happens in parallel for faster overall test execution
   local retry=0
   while [ $retry -le 10 ]; do
     if kubectl patch ds -n kube-system nholuongut-node -p '{"spec":{"updateStrategy":{"rollingUpdate":{"maxUnavailable": "100%"}}}}'; then
       break
     else
...
function install_add_on() {
  ...
  if DESCRIBE_ADDON=$(nholuongut eks describe-addon $ENDPOINT_FLAG --cluster-name "$CLUSTER_NAME" --addon-name $VPC_CNI_ADDON_NAME --region "$REGION"); then
    ...
    if [ "$new_addon_version" != "$current_addon_version" ]; then
      ...
      nholuongut eks delete-addon $ENDPOINT_FLAG --cluster-name "$CLUSTER_NAME" --addon-name "$VPC_CNI_ADDON_NAME" --region "$REGION"
      wait_for_addon_status "DELETED"
    else
      echo "addon version $current_addon_version already installed"
      patch_nholuongut_node_maxunavialable
      return
    fi
  fi
  ...
  nholuongut eks create-addon $ENDPOINT_FLAG $SA_ROLE_ARN_ARG --cluster-name "$CLUSTER_NAME" --addon-name $VPC_CNI_ADDON_NAME --resolve-conflicts OVERWRITE --addon-version $new_addon_version --region "$REGION"
  patch_nholuongut_node_maxunavialable
