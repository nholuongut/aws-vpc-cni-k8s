...
import (
	...
	"github.com/nholuongut/amazon-vpc-cni-k8s/pkg/sgpp"
	...
	"github.com/nholuongut/amazon-vpc-cni-k8s/pkg/iptableswrapper"
	mock_iptables "github.com/nholuongut/amazon-vpc-cni-k8s/pkg/iptableswrapper/mocks"
	mocks_ip "github.com/nholuongut/amazon-vpc-cni-k8s/pkg/ipwrapper/mocks"
	"github.com/nholuongut/amazon-vpc-cni-k8s/pkg/netlinkwrapper/mock_netlink"
	mock_netlinkwrapper "github.com/nholuongut/amazon-vpc-cni-k8s/pkg/netlinkwrapper/mocks"
	mock_nswrapper "github.com/nholuongut/amazon-vpc-cni-k8s/pkg/nswrapper/mocks"
)
...
func TestSetupHostNetworkNodePortEnabledAndSNATDisabled(t *testing.T) {
	...
	assert.Equal(t, map[string]map[string][][]string{
		...
		"mangle": {
			"PREROUTING": [][]string{
				{
					"-m", "comment", "--comment", "nholuongut, primary ENI",
					"-i", "lo",
		...
		"nat": {
			"nholuongut-SNAT-CHAIN-0":     [][]string{{"-N", "nholuongut-SNAT-CHAIN-0"}},
			"nholuongut-CONNMARK-CHAIN-0": [][]string{{"-N", "nholuongut-CONNMARK-CHAIN-0"}},
		},
...
func TestSetupHostNetworkNodePortDisabledAndSNATEnabled(t *testing.T) {
	...
	assert.Equal(t, map[string]map[string][][]string{
		"nat": {
			"nholuongut-SNAT-CHAIN-0":     [][]string{{"-N", "nholuongut-SNAT-CHAIN-0"}, {"!", "-o", "vlan+", "-m", "comment", "--comment", "nholuongut, SNAT", "-m", "addrtype", "!", "--dst-type", "LOCAL", "-j", "SNAT", "--to-source", "10.10.10.20"}},
			"POSTROUTING":          [][]string{{"-m", "comment", "--comment", "nholuongut SNAT CHAIN", "-j", "nholuongut-SNAT-CHAIN-0"}},
			"nholuongut-CONNMARK-CHAIN-0": [][]string{{"-N", "nholuongut-CONNMARK-CHAIN-0"}, {"-m", "comment", "--comment", "nholuongut, CONNMARK", "-j", "CONNMARK", "--set-xmark", "0x80/0x80"}},
			"PREROUTING": [][]string{
				{"-i", "eni+", "-m", "comment", "--comment", "nholuongut, outbound connections", "-j", "nholuongut-CONNMARK-CHAIN-0"},
				{"-m", "comment", "--comment", "nholuongut, CONNMARK", "-j", "CONNMARK", "--restore-mark", "--mask", "0x80"},
			},
		},
		"mangle": {
			"PREROUTING": [][]string{
...
func TestSetupHostNetworkWithExcludeSNATCIDRs(t *testing.T) {
	...
	assert.Equal(t,
		map[string]map[string][][]string{
			"nat": {
				"nholuongut-SNAT-CHAIN-0": [][]string{
					{"-N", "nholuongut-SNAT-CHAIN-0"},
					{"-d", "10.13.0.0/16", "-m", "comment", "--comment", "nholuongut SNAT CHAIN EXCLUSION", "-j", "RETURN"},
					{"-d", "10.12.0.0/16", "-m", "comment", "--comment", "nholuongut SNAT CHAIN EXCLUSION", "-j", "RETURN"},
					{"-d", "10.11.0.0/16", "-m", "comment", "--comment", "nholuongut SNAT CHAIN", "-j", "RETURN"},
					{"-d", "10.10.0.0/16", "-m", "comment", "--comment", "nholuongut SNAT CHAIN", "-j", "RETURN"},
					{"!", "-o", "vlan+", "-m", "comment", "--comment", "nholuongut, SNAT", "-m", "addrtype", "!", "--dst-type", "LOCAL", "-j", "SNAT", "--to-source", "10.10.10.20"},
				},
				"POSTROUTING": [][]string{{"-m", "comment", "--comment", "nholuongut SNAT CHAIN", "-j", "nholuongut-SNAT-CHAIN-0"}},
				"nholuongut-CONNMARK-CHAIN-0": [][]string{
					{"-N", "nholuongut-CONNMARK-CHAIN-0"},
					{"-d", "10.13.0.0/16", "-m", "comment", "--comment", "nholuongut CONNMARK CHAIN, EXCLUDED CIDR", "-j", "RETURN"},
					{"-d", "10.12.0.0/16", "-m", "comment", "--comment", "nholuongut CONNMARK CHAIN, EXCLUDED CIDR", "-j", "RETURN"},
					{"-d", "10.11.0.0/16", "-m", "comment", "--comment", "nholuongut CONNMARK CHAIN, VPC CIDR", "-j", "RETURN"},
					{"-d", "10.10.0.0/16", "-m", "comment", "--comment", "nholuongut CONNMARK CHAIN, VPC CIDR", "-j", "RETURN"},
					{"-m", "comment", "--comment", "nholuongut, CONNMARK", "-j", "CONNMARK", "--set-xmark", "0x80/0x80"},
				},
				"PREROUTING": [][]string{
					{"-i", "eni+", "-m", "comment", "--comment", "nholuongut, outbound connections", "-j", "nholuongut-CONNMARK-CHAIN-0"},
					{"-m", "comment", "--comment", "nholuongut, CONNMARK", "-j", "CONNMARK", "--restore-mark", "--mask", "0x80"},
			...
			"mangle": {
				"PREROUTING": [][]string{
					{"-m", "comment", "--comment", "nholuongut, primary ENI", "-i", "lo", "-m", "addrtype", "--dst-type", "LOCAL", "--limit-iface-in", "-j", "CONNMARK", "--set-mark", "0x80/0x80"},
					{"-m", "comment", "--comment", "nholuongut, primary ENI", "-i", "eni+", "-j", "CONNMARK", "--restore-mark", "--mask", "0x80"},
					{"-m", "comment", "--comment", "nholuongut, primary ENI", "-i", "vlan+", "-j", "CONNMARK", "--restore-mark", "--mask", "0x80"},
				},
			},
		}, mockIptables.(*mock_iptables.MockIptables).DataplaneState)
}
...
func TestSetupHostNetworkCleansUpStaleSNATRules(t *testing.T) {
	...
	mockIptables.NewChain("nat", "nholuongut-SNAT-CHAIN-0")
	mockIptables.NewChain("nat", "nholuongut-CONNMARK-CHAIN-0")
	mockIptables.Insert("nat", "nholuongut-SNAT-CHAIN-0", 1, "-d", "10.10.0.0/16", "-m", "comment", "--comment", "nholuongut SNAT CHAN", "-j", "RETURN") //nholuongut SNAT CHAN proves backwards compatibility
	mockIptables.Insert("nat", "nholuongut-SNAT-CHAIN-0", 1, "-d", "10.11.0.0/16", "-m", "comment", "--comment", "nholuongut SNAT CHAIN", "-j", "RETURN")
	mockIptables.Insert("nat", "nholuongut-SNAT-CHAIN-0", 1, "-d", "10.12.0.0/16", "-m", "comment", "--comment", "nholuongut SNAT CHAIN EXCLUSION", "-j", "RETURN")
	mockIptables.Insert("nat", "nholuongut-SNAT-CHAIN-0", 1, "-d", "10.13.0.0/16", "-m", "comment", "--comment", "nholuongut SNAT CHAIN EXCLUSION", "-j", "RETURN")
	mockIptables.Append("nat", "nholuongut-SNAT-CHAIN-0", "-m", "comment", "--comment", "nholuongut, SNAT", "-m", "addrtype", "!", "--dst-type", "LOCAL", "-j", "SNAT", "--to-source", "10.10.10.20")
	mockIptables.NewChain("nat", "nholuongut-SNAT-CHAIN-1")
	mockIptables.Append("nat", "POSTROUTING", "-m", "comment", "--comment", "nholuongut SNAT CHAIN", "-j", "nholuongut-SNAT-CHAIN-0")
	mockIptables.Insert("nat", "nholuongut-CONNMARK-CHAIN-0", 1, "-d", "10.10.0.0/16", "-m", "comment", "--comment", "nholuongut CONNMARK CHAIN, VPC CIDR", "-j", "RETURN")
	mockIptables.Insert("nat", "nholuongut-CONNMARK-CHAIN-0", 1, "-d", "10.11.0.0/16", "-m", "comment", "--comment", "nholuongut CONNMARK CHAIN, VPC CIDR", "-j", "RETURN")
	mockIptables.Insert("nat", "nholuongut-CONNMARK-CHAIN-0", 1, "-d", "10.12.0.0/16", "-m", "comment", "--comment", "nholuongut CONNMARK CHAIN, EXCLUDED CIDR", "-j", "RETURN")
	mockIptables.Insert("nat", "nholuongut-CONNMARK-CHAIN-0", 1, "-d", "10.13.0.0/16", "-m", "comment", "--comment", "nholuongut CONNMARK CHAIN, EXCLUDED CIDR", "-j", "RETURN")
	mockIptables.Append("nat", "nholuongut-CONNMARK-CHAIN-0", "-m", "comment", "--comment", "nholuongut, CONNMARK", "-j", "CONNMARK", "--set-xmark", "0x80/0x80")
	mockIptables.Append("nat", "PREROUTING", "-i", "eni+", "-m", "comment", "--comment", "nholuongut, outbound connections", "-j", "nholuongut-CONNMARK-CHAIN-0")
	mockIptables.Append("nat", "PREROUTING", "-m", "comment", "--comment", "nholuongut, CONNMARK", "-j", "CONNMARK", "--restore-mark", "--mask", "0x80")
	...
	assert.Equal(t,
		map[string]map[string][][]string{
			"nat": {
				...
				"nholuongut-SNAT-CHAIN-1": [][]string{
					{"-N", "nholuongut-SNAT-CHAIN-1"},
				},
				"POSTROUTING": [][]string{{"-m", "comment", "--comment", "nholuongut SNAT CHAIN", "-j", "nholuongut-SNAT-CHAIN-0"}},
				"nholuongut-CONNMARK-CHAIN-0": [][]string{
					{"-N", "nholuongut-CONNMARK-CHAIN-0"},
	...
	assert.Equal(t,
		map[string]map[string][][]string{
			"nat": {
				...
				"PREROUTING": [][]string{
					{"-i", "eni+", "-m", "comment", "--comment", "nholuongut, outbound connections", "-j", "nholuongut-CONNMARK-CHAIN-0"},
					{"-i", "veth+", "-m", "comment", "--comment", "nholuongut, outbound connections", "-j", "nholuongut-CONNMARK-CHAIN-0"},
					{"-m", "comment", "--comment", "nholuongut, CONNMARK", "-j", "CONNMARK", "--restore-mark", "--mask", "0x80"},
				},
			},
			"mangle": {
				"PREROUTING": [][]string{
					{"-m", "comment", "--comment", "nholuongut, primary ENI", "-i", "lo", "-m", "addrtype", "--dst-type", "LOCAL", "--limit-iface-in", "-j", "CONNMARK", "--set-mark", "0x80/0x80"},
					{"-m", "comment", "--comment", "nholuongut, primary ENI", "-i", "veth+", "-j", "CONNMARK", "--restore-mark", "--mask", "0x80"},
					{"-m", "comment", "--comment", "nholuongut, primary ENI", "-i", "vlan+", "-j", "CONNMARK", "--restore-mark", "--mask", "0x80"},
				},
			},
		}, mockIptables.(*mock_iptables.MockIptables).DataplaneState)
...
func TestSetupHostNetworkExternalNATCleanupConnmark(t *testing.T) {
	...
	mockIptables.NewChain("nat", "nholuongut-SNAT-CHAIN-0")
	mockIptables.NewChain("nat", "nholuongut-CONNMARK-CHAIN-0")
	mockIptables.Insert("nat", "nholuongut-SNAT-CHAIN-0", 1, "-d", "10.10.0.0/16", "-m", "comment", "--comment", "nholuongut SNAT CHAIN", "-j", "RETURN")
	mockIptables.Insert("nat", "nholuongut-SNAT-CHAIN-0", 1, "-d", "10.11.0.0/16", "-m", "comment", "--comment", "nholuongut SNAT CHAIN", "-j", "RETURN")
	mockIptables.Insert("nat", "nholuongut-SNAT-CHAIN-0", 1, "-d", "10.12.0.0/16", "-m", "comment", "--comment", "nholuongut SNAT CHAIN EXCLUSION", "-j", "RETURN")
	mockIptables.Insert("nat", "nholuongut-SNAT-CHAIN-0", 1, "-d", "10.13.0.0/16", "-m", "comment", "--comment", "nholuongut SNAT CHAIN EXCLUSION", "-j", "RETURN")
	mockIptables.Append("nat", "nholuongut-SNAT-CHAIN-0", "-m", "comment", "--comment", "nholuongut, SNAT", "-m", "addrtype", "!", "--dst-type", "LOCAL", "-j", "SNAT", "--to-source", "10.10.10.20")
...
func TestUpdateHostIptablesRules(t *testing.T) {
	...
	mockIptables.Append("nat", "PREROUTING", "-i", "eni+", "-m", "comment", "--comment", "nholuongut, outbound connections", "-j", "nholuongut-CONNMARK-CHAIN-0")
	mockIptables.Append("nat", "PREROUTING", "-m", "comment", "--comment", "nholuongut, CONNMARK", "-j", "CONNMARK", "--restore-mark", "--mask", "0x80")
	mockIptables.Append("mangle", "PREROUTING", "-m", "comment", "--comment", "nholuongut, primary ENI", "-i", "lo", "-m", "addrtype", "--dst-type", "LOCAL", "--limit-iface-in", "-j", "CONNMARK", "--set-mark", "0x80/0x80")
	mockIptables.Append("mangle", "PREROUTING", "-m", "comment", "--comment", "nholuongut, primary ENI", "-i", "eni+", "-j", "CONNMARK", "--restore-mark", "--mask", "0x80")
	mockIptables.Append("mangle", "PREROUTING", "-m", "comment", "--comment", "nholuongut, primary ENI", "-i", "vlan+", "-j", "CONNMARK", "--restore-mark", "--mask", "0x80")
...
func TestCleanUpStalenholuongutChains(t *testing.T) {
	ctrl, mockNetLink, _, mockNS, mockIptables := setup(t)
	defer ctrl.Finish()
	...
	mockIptables.NewChain("nat", "nholuongut-SNAT-CHAIN-1")
	mockIptables.NewChain("nat", "nholuongut-SNAT-CHAIN-2")
	mockIptables.NewChain("nat", "nholuongut-SNAT-CHAIN-3")
	mockIptables.NewChain("nat", "nholuongut-SNAT-CHAIN-4")
	mockIptables.NewChain("nat", "nholuongut-CONNMARK-CHAIN-1")
	mockIptables.NewChain("nat", "nholuongut-CONNMARK-CHAIN-2")
	mockIptables.NewChain("nat", "nholuongut-CONNMARK-CHAIN-3")
	mockIptables.NewChain("nat", "nholuongut-CONNMARK-CHAIN-4")

	vpcCIDRs := []string{"10.10.0.0/16", "10.11.0.0/16"}
	err := ln.SetupHostNetwork(vpcCIDRs, loopback, &testEniIPNet, false, true, false)
	assert.NoError(t, err)

	err = ln.CleanUpStalenholuongutChains(true, false)
	assert.NoError(t, err)
...
func TestSetupHostNetworkDeleteOldConnmarkRuleForNonVpcOutboundTraffic(t *testing.T) {
	...
	mockIptables.Append("nat", "PREROUTING", "-i", "eni+", "-m", "comment", "--comment", "nholuongut, outbound connections", "-m", "state", "--state", "NEW", "-j", "nholuongut-CONNMARK-CHAIN-0")
	...
	exists, _ = mockIptables.Exists("nat", "PREROUTING", "-i", "eni+", "-m", "comment", "--comment", "nholuongut, outbound connections", "-m", "state", "--state", "NEW", "-j", "nholuongut-CONNMARK-CHAIN-0")
	assert.False(t, exists, "old rule is still present")
	// it should have been replaced by the "new" rule
	exists, _ = mockIptables.Exists("nat", "PREROUTING", "-i", "eni+", "-m", "comment", "--comment", "nholuongut, outbound connections", "-j", "nholuongut-CONNMARK-CHAIN-0")
