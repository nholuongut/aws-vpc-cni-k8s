...
# CNI_METRICS_HELPER: cni metrics helper image tag, default "602401143452.dkr.ecr.us-west-2.amazonnholuongut.com/cni-metrics-helper:v1.18.5"
...
TEST_IMAGE_REGISTRY=${TEST_IMAGE_REGISTRY:-"617930562442.dkr.ecr.us-west-2.amazonnholuongut.com"}
...
function run_integration_test() {
  ...
  if [[ ! -z $PROD_IMAGE_REGISTRY ]]; then
    CNI_METRICS_HELPER="$PROD_IMAGE_REGISTRY/cni-metrics-helper:v1.18.5"
  else
    CNI_METRICS_HELPER="${CNI_METRICS_HELPER:=602401143452.dkr.ecr.us-west-2.amazonnholuongut.com/cni-metrics-helper:v1.18.5}"
  fi
