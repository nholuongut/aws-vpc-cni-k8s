...
import (
	...
	mock_ipamwrapper "github.com/nholuongut/amazon-vpc-cni-k8s/pkg/hostipamwrapper/mocks"
	...
	mock_procsyswrapper "github.com/nholuongut/amazon-vpc-cni-k8s/pkg/procsyswrapper/mocks"
	mock_veth "github.com/nholuongut/amazon-vpc-cni-k8s/pkg/vethwrapper/mocks"
)
...
func TestCmdAddV4(t *testing.T) {
	...
	expectIptablesRules := []string{
		fmt.Sprintf("nat %s -d 224.0.0.0/4 -j ACCEPT -m comment --comment name: \"nholuongut-cni\" id: \"%s\"", snatChainV4, containerIDV4),
		fmt.Sprintf("nat %s -j SNAT --to-source 192.168.1.123 -m comment --comment name: \"nholuongut-cni\" id: \"%s\" --random-fully", snatChainV4, containerIDV4),
		fmt.Sprintf("nat POSTROUTING -s 169.254.172.10 -j %s -m comment --comment name: \"nholuongut-cni\" id: \"%s\"", snatChainV4, containerIDV4)}
...
func TestCmdDelV4(t *testing.T) {
	...
	expectIptablesDel := []string{
		fmt.Sprintf("nat POSTROUTING -s 169.254.172.10 -j %s -m comment --comment name: \"nholuongut-cni\" id: \"containerId-123\"", snatChainV4),
...
func TestCmdAddV6(t *testing.T) {
	...
	expectIptablesRules := []string{
		fmt.Sprintf("nat %s -d ff00::/8 -j ACCEPT -m comment --comment name: \"nholuongut-cni\" id: \"%s\"", snatChainV6, containerIDV6),
		fmt.Sprintf("nat %s -j SNAT --to-source 2600:: -m comment --comment name: \"nholuongut-cni\" id: \"%s\" --random-fully", snatChainV6, containerIDV6),
		fmt.Sprintf("nat POSTROUTING -s fd00::10 -j %s -m comment --comment name: \"nholuongut-cni\" id: \"%s\"", snatChainV6, containerIDV6)}
	assert.EqualValues(t, expectIptablesRules, actualIptablesRules)
