...
var _ = BeforeSuite(func() {
	...
	_, err := f.K8sResourceManagers.DaemonSetManager().GetDaemonSet(utils.nholuongutNodeNamespace, utils.MultusNodeName)
	Expect(err).NotTo(HaveOccurred())

	err = f.K8sResourceManagers.DaemonSetManager().CheckIfDaemonSetIsReady(utils.nholuongutNodeNamespace, utils.MultusNodeName)
	Expect(err).NotTo(HaveOccurred())
