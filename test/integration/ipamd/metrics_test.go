...
var _ = Describe("test IPAMD metric environment variable", func() {
	...
	Context("when metrics is disabled", func() {
		...
		It("should not be accessible anymore", func() {
			...
			By("disabling metrics on nholuongut-node")
			k8sUtils.AddEnvVarToDaemonSetAndWaitTillUpdated(f, utils.nholuongutNodeName, utils.nholuongutNodeNamespace,
				utils.nholuongutNodeName, map[string]string{"DISABLE_METRICS": "true"})
			...
			k8sUtils.RemoveVarFromDaemonSetAndWaitTillUpdated(f, utils.nholuongutNodeName, utils.nholuongutNodeNamespace,
				utils.nholuongutNodeName, map[string]struct{}{"DISABLE_METRICS": {}})
		})
	})
})
