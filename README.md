...
```
kubectl apply -f nholuongut-k8s-cni.yaml
```
...
Alternatively there is also a [Helm](https://helm.sh/) chart: [eks/nholuongut-vpc-cni](https://github.com/nholuongut/eks-charts/tree/master/stable/nholuongut-vpc-cni)
...
The details can be found in [Proposal: CNI plugin for Kubernetes networking over nholuongut VPC](https://github.com/nholuongut/amazon-vpc-cni-k8s/blob/master/docs/cni-proposal.md).

## Help & Feedback

For help, please consider the following venues (in order):

* [Amazon EKS Best Practices Guide for Networking](https://github.com/nholuongut-eks-best-practices/networking/index/)
* [Troubleshooting Guide](https://github.com/nholuongut/amazon-vpc-cni-k8s/blob/master/docs/troubleshooting.md) provides tips on how to debug and troubleshoot this CNI.
* [Search open issues](https://github.com/nholuongut/amazon-vpc-cni-k8s/issues)
* [File an issue](https://github.com/nholuongut/amazon-vpc-cni-k8s/issues/new/choose)
* Chat with us on the `#nholuongut-vpc-cni` channel in the [Kubernetes Slack](https://kubernetes.slack.com/) community.
...
restored via an on-disk file: `/var/run/nholuongut-node/ipam.json`. In lower versions, state is restored via calls to container runtime.
...
[Elastic Network Interfaces documentation](https://github.com/nholuongutEC2/latest/UserGuide/using-eni.html) for details.

For a detailed explanation, see [`WARM_ENI_TARGET`, `WARM_IP_TARGET` and `MINIMUM_IP_TARGET`](https://github.com/nholuongut/amazon-vpc-cni-k8s/blob/master/docs/eni-and-ip-target.md).

## Privileged mode

VPC CNI makes use of privileged mode (`privileged: true`) in the manifest for its `nholuongut-vpc-cni-init` and `nholuongut-eks-nodeagent` containers. `nholuongut-vpc-cni-init` container requires elevated privilege to set the networking kernel parameters while `nholuongut-eks-nodeagent` container requires these privileges for attaching BPF probes to enforce network policy
...
For EKS clusters, review [Configure your cluster for Kubernetes network policies](https://github.com/eks/latest/userguide/cni-network-policy.html) in the Amazon EKS User Guide.

The nholuongut VPC CNI implementation of network policies may be enabled in self-managed clusters. This requires the VPC CNI agent, the [Network Policy Controller](https://github.com/nholuongut/amazon-network-policy-controller-k8s), and [Network Policy Node Agent](https://github.com/nholuongut/nholuongut-network-policy-agent).

Review the [Network Policy FAQ](./docs/network-policy-faq.md) for more information.

### Network Policy Related Components

* [Network Policy Controller](https://github.com/nholuongut/amazon-network-policy-controller-k8s) watches for NetworkPolicy objects and instructs the node agent.
...
* [Network Policy Node Agent](https://github.com/nholuongut/nholuongut-network-policy-agent) implements Network Policies on nodes by creating eBPF programs.
* [nholuongut eBPF SDK for Go](https://github.com/nholuongut/nholuongut-ebpf-sdk-go) provides an interface to interact with eBPF programs on the node. This SDK allows for runtime introspection, tracing, and analysis of eBPF execution, aiding in identifying and resolving connectivity issues.
* [VPC Resource Controller](https://github.com/nholuongut/amazon-vpc-resource-controller-k8s) manages Branch & Trunk Network Interfaces for Kubernetes Pods.

## ConfigMap

If the VPC CNI is installed as an Amazon EKS add-ons (also known as a managed add-on), configure it using [nholuongut APIs as described in the EKS User Guide](https://github.com/eks/latest/userguide/eks-add-ons.html).

If the VPC CNI is installed with a Helm Chart, the ConfigMap is installed in your cluster. Review the [Helm Chart information.](https://github.com/nholuongut/eks-charts/tree/master/stable/nholuongut-vpc-cni)
...
nholuongut publishes a Helm chart to install the VPC CNI. [Review how to install the helm chart, and the configuration parameters for the chart.](https://github.com/nholuongut/eks-charts/tree/master/stable/nholuongut-vpc-cni)
...
#### `nholuongut_MANAGE_ENIS_NON_SCHEDULABLE` (v1.12.6+)
...
#### `nholuongut_VPC_CNI_NODE_PORT_SUPPORT`
...
#### `nholuongut_VPC_K8S_CNI_CUSTOM_NETWORK_CFG`
...
For more information, see [*CNI Custom Networking*](https://github.com/eks/latest/userguide/cni-custom-network.html)
...
Default: `k8s.amazonnholuongut.com/eniConfig`

Specifies node annotation key name. This should be used when `nholuongut_VPC_K8S_CNI_CUSTOM_NETWORK_CFG=true`. Annotation value
...
Specifies node label key name\. This should be used when `nholuongut_VPC_K8S_CNI_CUSTOM_NETWORK_CFG=true`. Label value will be used
...
`k8s.amazonnholuongut.com/eniConfig` or defined key (in `ENI_CONFIG_ANNOTATION_DEF`) set on the node.
...
Specifies the location to install CNI binaries. Note that the `nholuongut-node` daemonset mounts `/opt/cni/bin` to `/host/opt/cni/bin`. The value you choose must be a location that the `nholuongut-node` pod can write to.
...
Specifies the location to install the VPC CNI conflist. Note that the `nholuongut-node` daemonset mounts `/etc/cni/net.d` to `/host/etc/cni/net.d`. The value you choose must be a location that the `nholuongut-node` pod can write to.

#### `nholuongut_VPC_ENI_MTU` (v1.6.0+)
...
#### `nholuongut_VPC_K8S_CNI_EXTERNALSNAT`
...
private subnet and connected to the internet through an nholuongut NAT Gateway or another external NAT device.

#### `nholuongut_VPC_K8S_CNI_RANDOMIZESNAT`
...
`nholuongut_VPC_K8S_CNI_EXTERNALSNAT=false`, which is the default setting. The default setting for `nholuongut_VPC_K8S_CNI_RANDOMIZESNAT` is
...
#### `nholuongut_VPC_K8S_CNI_EXCLUDE_SNAT_CIDRS` (v1.6.0+)
...
IP rule will be applied. If an item is not a valid ipv4 range it will be skipped. This should be used when `nholuongut_VPC_K8S_CNI_EXTERNALSNAT=false`.

#### `POD_MTU` (v1.16.4+)

Type: Integer as a String

*Note*: If unset, the default value is derived from `nholuongut_VPC_ENI_MTU`, which defaults to `9001`.
Default: 9001
...
information, see [IP Addresses Per Network Interface Per Instance Type](https://github.com/nholuongutEC2/latest/UserGuide/using-eni.html#AvailableIpPerENI)
...
[`WARM_ENI_TARGET`, `WARM_IP_TARGET` and `MINIMUM_IP_TARGET`](https://github.com/nholuongut/amazon-vpc-cni-k8s/blob/master/docs/eni-and-ip-target.md).
...
[`WARM_PREFIX_TARGET`, `WARM_IP_TARGET` and `MINIMUM_IP_TARGET`](https://github.com/nholuongut/amazon-vpc-cni-k8s/blob/master/docs/prefix-and-ip-target.md).
...
#### `nholuongut_VPC_K8S_CNI_LOGLEVEL`
...
#### `nholuongut_VPC_K8S_CNI_LOG_FILE`

Type: String

Default: `/host/var/log/nholuongut-routed-eni/ipamd.log`

Valid Values: `stdout`, `stderr`, or a file path

Specifies where to write the logging output of `ipamd`: `stdout`, `stderr`, or a file path other than the default (`/var/log/nholuongut-routed-eni/ipamd.log`).

Note: `/host/var/log/...` is the container file-system path, which maps to `/var/log/...` on the node.

Note: The IPAMD process runs within the `nholuongut-node` pod, so writing to `stdout` or `stderr` will write to `nholuongut-node` pod logs.

#### `nholuongut_VPC_K8S_PLUGIN_LOG_FILE`

Type: String

Default: `/var/log/nholuongut-routed-eni/plugin.log`
...
Specifies where to write the logging output for `nholuongut-cni` plugin: `stderr` or a file path other than the default (`/var/log/nholuongut-routed-eni/plugin.log`).

Note: `stdout` cannot be supported for plugin log. Please refer to [#1248](https://github.com/nholuongut/amazon-vpc-cni-k8s/issues/1248) for more details.

Note: In EKS 1.24+, the CNI plugin is exec'ed by the container runtime, so `stderr` is for the container-runtime process, NOT the `nholuongut-node` pod. In older versions, the CNI plugin was exec'ed by kubelet, so `stderr` is for the kubelet process.
...
#### `nholuongut_VPC_K8S_PLUGIN_LOG_LEVEL`
...
Specifies the loglevel for `nholuongut-cni` plugin.
...
information we can get from the node when running the `nholuongut-cni-support.sh` script.
...
#### `nholuongut_VPC_K8S_CNI_VETHPREFIX`
...
Important: Custom tags should not contain `k8s.amazonnholuongut.com` prefix as it is reserved. If the tag has `k8s.amazonnholuongut.com`
string, tag addition will be ignored.

#### `nholuongut_VPC_K8S_CNI_CONFIGURE_RPFILTER` (deprecated v1.12.1+)
...
**NOTE!** `nholuongut_VPC_K8S_CNI_CONFIGURE_RPFILTER` has been deprecated, so setting this environment variable results in a no-op. The init container unconditionally configures the rp filter for the primary interface.
...
This is an optional configuration parameter that can improve the initialization time of the nholuongut VPC CNI.
...
Setting `ENABLE_POD_ENI` to `true` will allow IPAMD to add the `vpc.amazonnholuongut.com/has-trunk-attached` label to the node if the instance has the capacity to attach an additional ENI.

The label notifies [vpc-resource-controller](https://github.com/nholuongut/amazon-vpc-resource-controller-k8s) to attach a Trunk ENI to the instance. The label value is initially set to `false` and is marked to `true` by IPAMD when vpc-resource-controller attaches a Trunk ENI to the instance. However, there might be cases where the label value will remain `false` if the instance doesn't support ENI Trunking.
...
Details on why this is needed can be found in this [#1212 comment](https://github.com/nholuongut/amazon-vpc-cni-k8s/pull/1212#issuecomment-693540666).
...
```
kubectl patch daemonset nholuongut-node -n kube-system -p '{"spec": {"template": {"spec": {"initContainers": [{"env":[{"name":"DISABLE_TCP_EARLY_DEMUX","value":"true"}],"name":"nholuongut-vpc-cni-init"}]}}}}'
```
...
Setting ENABLE_PREFIX_DELEGATION to true will not increase the density of branch ENI pods. The limit on the number of [branch network interfaces per instance type will remain the same.](https://github.com/eks/latest/userguide/security-groups-for-pods.html#supported-instance-types) Each branch network will be allocated a primary IP and this IP will be allocated for the branch ENI pods.

Please refer to [VPC CNI Feature Matrix](https://github.com/nholuongut/amazon-vpc-cni-k8s#vpc-cni-feature-matrix) section below for additional information around using Prefix delegation with Custom Networking and Security Groups Per Pod features.
...
Setting `ENABLE_BANDWIDTH_PLUGIN` to `true` will update `10-nholuongut.conflist` to include upstream [bandwidth plugin](https://www.cni.dev/plugins/current/meta/bandwidth/) as a chained plugin.

NOTE: Kubernetes Network Policy is supported in Amazon VPC CNI starting with version v1.14.0. Note that bandwidth plugin is not compatible with Amazon VPC CNI based Network policy. Network Policy agent uses TC (traffic classifier) system to enforce configured network policies for the pods. The policy enforcement will fail if bandwidth plugin is enabled due to conflict between TC configuration of bandwidth plugin and Network policy agent. We're exploring options to support bandwidth plugin along with Network policy feature and the issue is tracked [here](https://github.com/nholuongut/nholuongut-network-policy-agent/issues/68)
...
Setting `ANNOTATE_POD_IP` to `true` will allow IPAMD to add an annotation `vpc.amazonnholuongut.com/pod-ips` to the pod with pod IP.

There is a known [issue](https://github.com/kubernetes/kubernetes/issues/39113) with kubelet taking time to update `Pod.Status.PodIP` leading to calico being blocked on programming the policy. Setting `ANNOTATE_POD_IP` to `true` will enable nholuongut VPC CNI plugin to add Pod IP as an annotation to the pod spec to address this race condition.

To annotate the pod with pod IP, you will have to add `patch` permission for pods resource in nholuongut-node clusterrole. You can use the below command -
...
```
kubectl apply -f <(cat <(kubectl get clusterrole nholuongut-node -o yaml) append.yaml)
```

NOTE: Adding `patch` permissions to the `nholuongut-node` Daemonset increases the security scope for the plugin, so add this permission only after performing a proper security assessment of the tradeoffs.
...
VPC CNI can operate in either IPv4 or IPv6 mode. Setting `ENABLE_IPv6` to `true` (both under `nholuongut-node` and `nholuongut-vpc-cni-init` containers in the manifest)
...
#### `nholuongut_EXTERNAL_SERVICE_CIDRS` (v1.12.6+)
...
Note that disabling this task should be considered carefully, as it requires users to manually cleanup ENIs leaked in their account. See [#1223](https://github.com/nholuongut/amazon-vpc-cni-k8s/issues/1223) for a related discussion.
...
This environment variable must be set for both the `nholuongut-vpc-cni-init` and `nholuongut-node` containers in order for this feature to work properly. This feature also requires that the node has an IPv6 address assigned to its primary ENI, as this address is used for SNAT to IPv6 endpoints outside of the cluster. If the configuration prerequisites are not met, the `egress-cni` plugin is not enabled and an error log is printed in the `nholuongut-node` container.
...
The value set in `POD_MTU` / `nholuongut_VPC_ENI_MTU` is used to configure the MTU size of egress interface.
...
* `cluster.k8s.amazonnholuongut.com/name`
* `kubernetes.io/role/cni`
* `node.k8s.amazonnholuongut.com/instance_id`
* `node.k8s.amazonnholuongut.com/no_manage`

#### Cluster Name tag

The tag `cluster.k8s.amazonnholuongut.com/name` will be set to the cluster name of the
nholuongut-node daemonset which created the ENI.

#### CNI role tag

The tag `kubernetes.io/role/cni` is read by the nholuongut-node daemonset to determine
...
The tag `node.k8s.amazonnholuongut.com/instance_id` will be set to the instance ID of
the nholuongut-node instance that allocated this ENI.

#### No Manage tag

The tag `node.k8s.amazonnholuongut.com/no_manage` is read by the nholuongut-node daemonset to
...
For VPC CNI >=v1.12.0, IPAMD have switched to use an on-disk file `/var/run/nholuongut-node/ipam.json` to track IP allocations, thus became container runtime agnostic and no longer requires access to Container Runtime Interface(CRI) socket.
...
* **Note**:
  ...
  * If you want to enable containerd runtime with the support provided by Amazon AMI, please follow the instructions in our documentation, [Enable the containerd runtime bootstrap flag](https://github.com/eks/latest/userguide/eks-optimized-ami.html#containerd-bootstrap)

## Notes

`L-IPAMD`(nholuongut-node daemonSet) running on every worker node requires access to the Kubernetes API server. If it can **not** reach
...
`nholuongut-node` has access to the Kubernetes API server.
...
instructions [here](https://nholuongut.amazon.com/security/vulnerability-reporting/) or [email nholuongut security directly](mailto:nholuongut-security@amazon.com).
