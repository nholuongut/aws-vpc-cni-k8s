...
var _ = BeforeSuite(func() {
	...
	createSubnetOutput, err := f.CloudServices.EC2().
		CreateSubnet(subnetCidr.String(), f.Options.nholuongutVPCID, *primaryInstance.Placement.AvailabilityZone)
	Expect(err).ToNot(HaveOccurred())
	...
	k8sUtils.AddEnvVarToDaemonSetAndWaitTillUpdated(f, utils.nholuongutNodeName, utils.nholuongutNodeNamespace,
		utils.nholuongutNodeName, map[string]string{"WARM_ENI_TARGET": "0"})
...
var _ = AfterSuite(func() {
	...
	k8sUtils.AddEnvVarToDaemonSetAndWaitTillUpdated(f, utils.nholuongutNodeName, utils.nholuongutNodeNamespace,
		utils.nholuongutNodeName, map[string]string{"WARM_ENI_TARGET": "1"})
})
