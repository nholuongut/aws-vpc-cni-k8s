...
import (
	...
	"github.com/nholuongut/amazon-vpc-cni-k8s/test/framework/resources/agent"
	...
	"github.com/nholuongut/amazon-vpc-resource-controller-k8s/apis/vpcresources/v1beta1"
	vpcControllerFW "github.com/nholuongut/amazon-vpc-resource-controller-k8s/test/framework/manifest"
	...
	Context("when toggling DISABLE_TCP_EARLY_DEMUX", func() {
		...
		JustBeforeEach(func() {
			k8sUtils.AddEnvVarToDaemonSetAndWaitTillUpdated(f, utils.nholuongutNodeName,
				utils.nholuongutNodeNamespace, utils.nholuongutInitContainerName,
...
func ValidatePodsHaveBranchENI(podList v1.PodList) error {
	for _, pod := range podList.Items {
		if val, ok := pod.Annotations["vpc.amazonnholuongut.com/pod-eni"]; ok {
