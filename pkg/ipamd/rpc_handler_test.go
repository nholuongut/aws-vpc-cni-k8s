...
func TestServer_VersionCheck(t *testing.T) {
	...
	m.nholuongututils.EXPECT().GetVPCIPv4CIDRs().Return([]string{}, nil).AnyTimes()
	m.nholuongututils.EXPECT().GetVPCIPv6CIDRs().Return([]string{}, nil).AnyTimes()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, call := range tt.fields.getVPCIPv4CIDRsCalls {
				m.nholuongututils.EXPECT().GetVPCIPv4CIDRs().Return(call.cidrs, call.err)
			}
			for _, call := range tt.fields.getVPCIPv6CIDRsCalls {
				m.nholuongututils.EXPECT().GetVPCIPv6CIDRs().Return(call.cidrs, call.err)
			}
