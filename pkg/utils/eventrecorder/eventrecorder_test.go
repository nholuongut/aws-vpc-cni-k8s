...
func TestSendPodEvent(t *testing.T) {
	...
	MyPodName = "nholuongut-node-5test"
	mockEventRecorder := Get()

	labels := map[string]string{"k8s-app": "nholuongut-node"}
	pod := v1.Pod{
	...
	msg := "Failed to call ec2:DescribeNetworkInterfaces due to missing permissions. Please refer to https://github.com/nholuongut/amazon-vpc-cni-k8s/blob/master/docs/iam-policy.md"
