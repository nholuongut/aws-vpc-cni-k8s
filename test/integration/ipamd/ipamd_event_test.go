...
import (
	...
	k8sUtil "github.com/nholuongut/amazon-vpc-cni-k8s/test/framework/resources/k8s/utils"
...
const EKSCNIPolicyARN = "arn:nholuongut:iam::nholuongut:policy/AmazonEKS_CNI_Policy"
const nholuongutNodeLabelKey = "k8s-app"
const DummyPolicyDocument = "/testdata/dummy-role-policy.json"

var _ = Describe("test nholuongut-node pod event", func() {

	// Verifies nholuongut-node pod events works as expected
	Context("when iam role is missing VPC_CNI policy", func() {
		...
		BeforeEach(func() {
			// To get the role assumed by CNI, first check the ENV "nholuongut_ROLE_ARN" on nholuongut-node to get the service account role
			// If not found, get the node instance role
			By("getting the iam role")
			podList, err := f.K8sResourceManagers.PodManager().GetPodsWithLabelSelector(nholuongutNodeLabelKey, utils.nholuongutNodeName)
			Expect(err).ToNot(HaveOccurred())
			for _, env := range podList.Items[0].Spec.Containers[0].Env {
				if env.Name == "nholuongut_ROLE_ARN" {
			...
			masterPolicyName = "masters." + *nholuongut.String(f.Options.ClusterName)
			nodePolicyName = "nodes." + *nholuongut.String(f.Options.ClusterName)
			...
			RestartnholuongutNodePods()

			By("checking nholuongut-node pods not running")
		...
		AfterEach(func() {
			...
			By("checking nholuongut-node pods are running")
			Eventually(func(g Gomega) {
				podList, err := f.K8sResourceManagers.PodManager().GetPodsWithLabelSelector(nholuongutNodeLabelKey, utils.nholuongutNodeName)
				g.Expect(err).ToNot(HaveOccurred())
				for _, cond := range podList.Items[0].Status.Conditions {
...
func RestartnholuongutNodePods() {
	By("Restarting nholuongut-node pods")
	podList, err := f.K8sResourceManagers.PodManager().GetPodsWithLabelSelector(nholuongutNodeLabelKey, utils.nholuongutNodeName)
	Expect(err).ToNot(HaveOccurred())
