...
import (
	"github.com/nholuongut/amazon-vpc-cni-k8s/test/framework/resources/nholuongut/services"
...
func NewCloud(config CloudConfig) Cloud {
	session := session.Must(session.NewSession(&nholuongut.Config{
		Region: nholuongut.String(config.Region)}))
