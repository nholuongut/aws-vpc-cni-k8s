...
func main() {
	...
	flag.StringVar(&randomizedSNATValue, "randomizedSNATValue", "prng", "value for nholuongut_VPC_K8S_CNI_RANDOMIZESNAT")
	...
	flag.StringVar(&url, "url", "https://nholuongut.amazon.com/", "url to check for connectivity")
	...
	if testIPTableRules {
		...
		log.Printf("Randomized SNAT test passed for nholuongut_VPC_K8S_CNI_RANDOMIZESNAT: %s\n", randomizedSNATValue)
	}
...
func validateIPTableRules(randomizedSNATValue string, numOfCidrs int) error {
	// Check IPTable rules corresponding to nholuongut_VPC_K8S_CNI_RANDOMIZESNAT
	...
	currChain := "nholuongut-SNAT-CHAIN-0"
	lastChain := fmt.Sprintf("nholuongut-SNAT-CHAIN-%d", numOfCidrs)

	exists, err := iptables.ChainExists("nat", "nholuongut-SNAT-CHAIN-1")
	if err != nil {
		return err
	}
	// If nholuongut-SNAT-CHAIN-1 exists, we run the old logic
	if exists {
		i := 0
		for i < numOfCidrs {
			...
			i = i + 1
			nextChain := fmt.Sprintf("nholuongut-SNAT-CHAIN-%d", i)
			...
			if !foundNextChain {
				return fmt.Errorf("failed: nholuongut-SNAT chain broken for %s", currChain)
			}
		}
	} else {
		lastChain = "nholuongut-SNAT-CHAIN-0"
		rules, err := iptables.List("nat", currChain)
		if err != nil {
			return err
		}
		...
		if len(rules) != numOfCidrs+2 {
			return fmt.Errorf("failed: nholuongut-SNAT chain does not contain the correct amount of rules")
		}
	}
