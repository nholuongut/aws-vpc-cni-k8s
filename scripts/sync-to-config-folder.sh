...
REPO="nholuongut/amazon-vpc-cni-k8s"
PR_ID=$(uuidgen | cut -d '-' -f1)

SYNC_DIR="${BUILD_DIR}/config-sync"

BINARY_BASE="nholuongut-vpc-cni"
INCLUDE_NOTES=0
MANUAL_VERIFY=1
...
RELEASE_BRANCH=$(curl -s -H "Authorization: token $GITHUB_TOKEN" \
    https://api.github.com/repos/nholuongut/amazon-vpc-cni-k8s/releases | \
...
gh repo clone nholuongut/amazon-vpc-cni-k8s
...
cp $SCRIPTPATH/../build/cni-rel-yamls/${VERSION}/nholuongut-k8s-cni*.yaml .
