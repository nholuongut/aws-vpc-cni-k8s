...
//
// In the case of the nholuongut VPC CNI plugin, we have two components to the plugin.
...
// nholuongut-k8s-agent daemon which houses the IPAM controller.
//
...
// the nholuongut VPC CNI plugin binary, we only want to copy the CNI plugin binary
...
// nholuongut VPC CNI entrypoint binary
package main
...
import (
	...
	"github.com/nholuongut/amazon-vpc-cni-k8s/utils/cp"
)
...
const (
	...
	defaultnholuongutconflistFile       = "/app/10-nholuongut.conflist"
	tmpnholuongutconflistFile           = "/tmp/10-nholuongut.conflist"
	...
	defaultPluginLogFile         = "/var/log/nholuongut-routed-eni/plugin.log"
	defaultEgressV4PluginLogFile = "/var/log/nholuongut-routed-eni/egress-v4-plugin.log"
	defaultEgressV6PluginLogFile = "/var/log/nholuongut-routed-eni/egress-v6-plugin.log"
	...
	nholuongutConflistFile              = "/10-nholuongut.conflist"
	...
	envEniMTU                = "nholuongut_VPC_ENI_MTU"
	envPodMTU                = "POD_MTU"
	envEnablePodEni          = "ENABLE_POD_ENI"
	envPodSGEnforcingMode    = "POD_SECURITY_GROUP_ENFORCING_MODE"
	envPluginLogFile         = "nholuongut_VPC_K8S_PLUGIN_LOG_FILE"
	envPluginLogLevel        = "nholuongut_VPC_K8S_PLUGIN_LOG_LEVEL"
	envEgressV4PluginLogFile = "nholuongut_VPC_K8S_EGRESS_V4_PLUGIN_LOG_FILE"
	envEgressV6PluginLogFile = "nholuongut_VPC_K8S_EGRESS_V6_PLUGIN_LOG_FILE"
...
func validateEnvVars() bool {
	...
	if pluginLogFile == "stdout" {
		log.Errorf("nholuongut_VPC_K8S_PLUGIN_LOG_FILE cannot be set to stdout")
		return false
	}
	...
	vethPrefix := utils.GetEnv(envVethPrefix, defaultVethPrefix)
	if len(vethPrefix) > 4 {
		log.Errorf("nholuongut_VPC_K8S_CNI_VETHPREFIX cannot be longer than 4 characters")
		return false
	}

	if vethPrefix == "eth" || vethPrefix == "lo" || vethPrefix == "vlan" {
		log.Errorf("nholuongut_VPC_K8S_CNI_VETHPREFIX cannot be set to reserved values 'eth', 'vlan', or 'lo'")
		return false
	}
...
func _main() int {
	log.Debug("Started nholuongut-node container")
	if !validateEnvVars() {
		return 1
	}

	pluginBins := []string{"nholuongut-cni", "egress-cni"}
	...
	cmd := "./nholuongut-k8s-agent"
	// Exec redirects stdout and stderr to /dev/null, redirecting to os.Stdout and os.Stderr is done explicitly.
	// This enables the output of the nholuongut-k8s-agent to be displayed in the kubectl logs for the nholuongut-node container via stdout and stderr.
	...
	err = generateJSON(defaultnholuongutconflistFile, tmpnholuongutconflistFile, getPrimaryIP)
	if err != nil {
		log.WithError(err).Errorf("Failed to generate 10-nholuongutconflist")
		return 1
	}

	hostCniConfDirPath := utils.GetEnv(envHostCniConfDirPath, defaultHostCniConfDirPath)
	err = cp.CopyFile(tmpnholuongutconflistFile, hostCniConfDirPath+nholuongutConflistFile)
	if err != nil {
		log.WithError(err).Errorf("Failed to copy %s", nholuongutConflistFile)
		return 1
	}
