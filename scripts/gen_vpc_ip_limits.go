...
func main() {
	...
	// Generate --max-pods file for nholuongutlabs/amazon-eks-ami
...
func describeInstanceTypes(region string, eniLimitMap map[string]vpc.InstanceTypeLimits) {
	...
	sess := session.Must(session.NewSessionWithOptions(session.Options{
		SharedConfigState: session.SharedConfigEnable,
		Config:            *nholuongut.NewConfig().WithRegion(region),
	}))
	_, err := sess.Config.Credentials.Get()
	if err != nil {
		log.Fatalf("Failed to get session credentials: %v", err)
	...
	for {
		...
		for _, info := range output.InstanceTypes {
			...
			if len(info.NetworkInfo.NetworkCards) > 1 {
				eniLimit = int(nholuongut.Int64Value(info.NetworkInfo.NetworkCards[*info.NetworkInfo.DefaultNetworkCardIndex].MaximumNetworkInterfaces))
			} else {
...
// https://github.com/nholuongutEC2/latest/UserGuide/using-eni.html#AvailableIpPerENI
...
{{- end }}
#
# Mapping is calculated from nholuongut EC2 API using the following formula:
# * First IP on each ENI is not used for pods
# * +2 for the pods that use host-networking (nholuongut CNI and kube-proxy)
#
