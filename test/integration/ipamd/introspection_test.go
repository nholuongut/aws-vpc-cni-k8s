...
var _ = Describe("test Environment Variables for IPAMD Introspection ", func() {
	...
	JustBeforeEach(func() {
		...
		By("enabling introspection on the nholuongut-node")
		k8sUtils.AddEnvVarToDaemonSetAndWaitTillUpdated(f, utils.nholuongutNodeName, utils.nholuongutNodeNamespace,
			utils.nholuongutNodeName, map[string]string{"DISABLE_INTROSPECTION": "false"})
	...
	Context("when disabling introspection by setting DISABLE_INTROSPECTION to true", func() {
		It("introspection should not work anymore", func() {
			...
			By("disabling introspection on the nholuongut-node")
			...
			// Set the ENV variable
			By("removing the DISABLE_INTROSPECTION on the nholuongut-node")
			k8sUtils.RemoveVarFromDaemonSetAndWaitTillUpdated(f, utils.nholuongutNodeName, utils.nholuongutNodeNamespace,
				utils.nholuongutNodeName, map[string]struct{}{"DISABLE_INTROSPECTION": {}})
		})
	})
	...
	Context("when changing introspection bind address", func() {
		It("the introspection API be available on new address", func() {
			...
			k8sUtils.AddEnvVarToDaemonSetAndWaitTillUpdated(f, utils.nholuongutNodeName, utils.nholuongutNodeNamespace,
				utils.nholuongutNodeName, map[string]string{"INTROSPECTION_BIND_ADDRESS": newAddr})
			...
			// Set the ENV variable
			By("removing the INTROSPECTION_BIND_ADDRESS on the nholuongut-node")
