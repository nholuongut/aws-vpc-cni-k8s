...
import (
	...
	mock_ec2wrapper "github.com/nholuongut/amazon-vpc-cni-k8s/pkg/ec2wrapper/mocks"
...
func TestnholuongutGetFreeDeviceNumberOnErr(t *testing.T) {
	ctrl, mockEC2 := setup(t)
	defer ctrl.Finish()
	...
	_, err := cache.nholuongutGetFreeDeviceNumber()
	assert.Error(t, err)
}

func TestnholuongutGetFreeDeviceNumberNoDevice(t *testing.T) {
	ctrl, mockEC2 := setup(t)
	defer ctrl.Finish()
	...
	for i := 0; i < maxENIs; i++ {
		...
		ec2ENI := &ec2.InstanceNetworkInterface{Attachment: &ec2.InstanceNetworkInterfaceAttachment{DeviceIndex: &deviceNums[i], NetworkCardIndex: nholuongut.Int64(0)}}
...
func TestGetENIAttachmentID(t *testing.T) {
	...
	attachmentID := nholuongut.String("foo-attach")
	testCases := []struct {
		name   string
		...
		},
		{
			"not found error",
			nil,
			nholuonguterr.New("InvalidNetworkInterfaceID.NotFound", "", nil),
			nil,
			ErrENINotFound,
		},
	}
	...
	for _, tc := range testCases {
		mockEC2.EXPECT().DescribeNetworkInterfacesWithContext(gomock.Any(), gomock.Any(), gomock.Any()).Return(tc.output, tc.nholuongutErr)
...
func TestDescribeAllENIs(t *testing.T) {
	...
	result := &ec2.DescribeNetworkInterfacesOutput{
		NetworkInterfaces: []*ec2.NetworkInterface{{
			TagSet: []*ec2.Tag{
				{Key: nholuongut.String("foo"), Value: nholuongut.String("foo-value")},
			},
			Attachment: &ec2.NetworkInterfaceAttachment{
				NetworkCardIndex: nholuongut.Int64(0),
			},
			NetworkInterfaceId: nholuongut.String(primaryeniID),
		}},
	}

	expectedError := nholuonguterr.New("InvalidNetworkInterfaceID.NotFound", "no 'eni-xxx'", nil)
	noMessageError := nholuonguterr.New("InvalidNetworkInterfaceID.NotFound", "no message", nil)
	...
	testCases := []struct {
		...
		{"Not found error", nil, maxENIEC2APIRetries, nholuonguterr.New("InvalidNetworkInterfaceID.NotFound", "no 'eni-xxx'", nil), expectedError},
		{"Not found, no message", nil, maxENIEC2APIRetries, nholuonguterr.New("InvalidNetworkInterfaceID.NotFound", "no message", nil), noMessageError},
	...
	for _, tc := range testCases {
		mockEC2.EXPECT().DescribeNetworkInterfacesWithContext(gomock.Any(), gomock.Any(), gomock.Any()).Times(tc.n).Return(result, tc.nholuongutErr)
		cache := &EC2InstanceMetadataCache{imds: TypedIMDS{mockMetadata}, ec2SVC: mockEC2}
...
func TestAllocENI(t *testing.T) {
	...
	subnetResult := &ec2.DescribeSubnetsOutput{
		Subnets: []*ec2.Subnet{{
			AvailableIpAddressCount: nholuongut.Int64(ipAddressCount),
			SubnetId:                nholuongut.String(subnetID),
			Tags: []*ec2.Tag{
				{
					Key:   nholuongut.String("kubernetes.io/role/cni"),
					Value: nholuongut.String("1"),
				},
			},
		}},
	}
...
func TestAllocENIWithIPAddressesAlreadyFull(t *testing.T) {
	...
	retErr := nholuonguterr.New("PrivateIpAddressLimitExceeded", "Too many IPs already allocated", nil)
...
func TestnholuongutAPIErrInc(t *testing.T) {
	// Reset metrics before test
	prometheusmetrics.nholuongutAPIErr.Reset()

	// Test case 1: nholuongut error
	nholuongutErr := nholuonguterr.New("InvalidParameterException", "The parameter is invalid", nil)
	nholuongutAPIErrInc("CreateNetworkInterface", nholuongutErr)

	// Verify metric was incremented with correct labels
	count := testutil.ToFloat64(prometheusmetrics.nholuongutAPIErr.With(prometheus.Labels{
		"api":   "CreateNetworkInterface",
		"error": "InvalidParameterException",
	}))
	assert.Equal(t, float64(1), count)

	// Test case 2: Non-nholuongut error
	regularErr := errors.New("some other error")
	nholuongutAPIErrInc("CreateNetworkInterface", regularErr)

	// Verify metric was not incremented for non-nholuongut error
	count = testutil.ToFloat64(prometheusmetrics.nholuongutAPIErr.With(prometheus.Labels{
		"api":   "CreateNetworkInterface",
		"error": "InvalidParameterException",
	}))
...
func TestDescribeInstanceTypes(t *testing.T) {
	...
	mockEC2.EXPECT().DescribeInstanceTypesWithContext(gomock.Any(), gomock.Any(), gomock.Any()).Return(&ec2.DescribeInstanceTypesOutput{
		InstanceTypes: []*ec2.InstanceTypeInfo{
			{InstanceType: nholuongut.String("not-there"), NetworkInfo: &ec2.NetworkInfo{
				MaximumNetworkInterfaces:  nholuongut.Int64(9),
				Ipv4AddressesPerInterface: nholuongut.Int64(99)},
			},
		},
		NextToken: nil,
	}, nil)
...
func TestAllocIPAddresses(t *testing.T) {
	...
	input := &ec2.AssignPrivateIpAddressesInput{
		...
		SecondaryPrivateIpAddressCount: nholuongut.Int64(5),
	}
	...
	input = &ec2.AssignPrivateIpAddressesInput{
		NetworkInterfaceId:             nholuongut.String(eniID),
		SecondaryPrivateIpAddressCount: nholuongut.Int64(49),
	}
...
func TestAllocIPAddressesAlreadyFull(t *testing.T) {
	...
	input := &ec2.AssignPrivateIpAddressesInput{
		NetworkInterfaceId:             nholuongut.String(eniID),
		SecondaryPrivateIpAddressCount: nholuongut.Int64(14),
	}
...
func TestAllocPrefixAddresses(t *testing.T) {
	...
	input := &ec2.AssignPrivateIpAddressesInput{
		NetworkInterfaceId: nholuongut.String(eniID),
		Ipv4PrefixCount:    nholuongut.Int64(1),
	}
	mockEC2.EXPECT().AssignPrivateIpAddressesWithContext(gomock.Any(), input, gomock.Any()).Return(nil, nil)
...
func TestEC2InstanceMetadataCache_cleanUpLeakedENIsInternal(t *testing.T) {
	...
	interfaces := []*ec2.NetworkInterface{{
		...
		TagSet: []*ec2.Tag{
			{Key: nholuongut.String(eniNodeTagKey), Value: nholuongut.String("test-value")},
		},
	}}
...
func TestEC2InstanceMetadataCache_buildENITags(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: map[string]string{
				"node.k8s.amazonnholuongut.com/instance_id": "i-xxxxx",
			},
		},
		{
			...
			want: map[string]string{
				"node.k8s.amazonnholuongut.com/instance_id": "i-xxxxx",
				"cluster.k8s.amazonnholuongut.com/name":     "awesome-cluster",
			},
		},
		{
							...
							Filters: []*ec2.Filter{
								{
									...
									Values: []*string{nholuongut.String("node.k8s.amazonnholuongut.com/instance_id")},
								},
								{
									Name:   nholuongut.String("status"),
									Values: []*string{nholuongut.String("available")},
								},
								{
									Name:   nholuongut.String("vpc-id"),
									Values: []*string{nholuongut.String(vpcID)},
								},
							},
							MaxResults: nholuongut.Int64(1000),
						},
						outputPages: []*ec2.DescribeNetworkInterfacesOutput{
							{
						...
						outputPages: []*ec2.DescribeNetworkInterfacesOutput{
							{
								NetworkInterfaces: []*ec2.NetworkInterface{
									{
										NetworkInterfaceId: nholuongut.String("eni-1"),
										Description:        nholuongut.String("nholuongut-K8S-i-xxxxx"),
										Status:             nholuongut.String("available"),
										TagSet: []*ec2.Tag{
											{
												Key:   nholuongut.String("node.k8s.amazonnholuongut.com/instance_id"),
												Value: nholuongut.String("i-xxxxx"),
											},
											{
												Key:   nholuongut.String("node.k8s.amazonnholuongut.com/createdAt"),
												Value: nholuongut.String(tenMinuteAgo.Format(time.RFC3339)),
											},
										},
									},
								},
						...
						outputPages: []*ec2.DescribeNetworkInterfacesOutput{
							{
								NetworkInterfaces: []*ec2.NetworkInterface{
									{
										NetworkInterfaceId: nholuongut.String("eni-1"),
										Description:        nholuongut.String("non-k8s-i-xxxxx"),
										Status:             nholuongut.String("available"),
										TagSet: []*ec2.Tag{
											{
												Key:   nholuongut.String("node.k8s.amazonnholuongut.com/instance_id"),
								...
								NetworkInterfaces: []*ec2.NetworkInterface{
									{
										...
										TagSet: []*ec2.Tag{
											...
											{
												Key:   nholuongut.String("node.k8s.amazonnholuongut.com/createdAt"),
												Value: nholuongut.String(now.Format(time.RFC3339)),
											},
										},
									},
								},
				...
				describeNetworkInterfacePagesCalls: []describeNetworkInterfacePagesCall{
					{
						input: &ec2.DescribeNetworkInterfacesInput{
							Filters: []*ec2.Filter{
								...
								{
									Name:   nholuongut.String("tag:cluster.k8s.amazonnholuongut.com/name"),
									Values: []*string{nholuongut.String("awesome-cluster")},
								},
							},
							MaxResults: nholuongut.Int64(1000),
						},
								...
								NetworkInterfaces: []*ec2.NetworkInterface{
									{
										...
										TagSet: []*ec2.Tag{
											...
											{
												Key:   nholuongut.String("cluster.k8s.amazonnholuongut.com/name"),
												Value: nholuongut.String("awesome-cluster"),
											},
										},
									},
								},
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				...
				createTagsCalls: []createTagsCall{
					{
						input: &ec2.CreateTagsInput{
							Resources: []*string{nholuongut.String("eni-xxxx")},
							Tags: []*ec2.Tag{
								{
									Key:   nholuongut.String("cluster.k8s.amazonnholuongut.com/name"),
									Value: nholuongut.String("awesome-cluster"),
								...
								{
									Key:   nholuongut.String("node.k8s.amazonnholuongut.com/instance_id"),
									Value: nholuongut.String("i-xxxx"),
								},
							},
						},
					},
		...
		},
		{
			...
			args: args{
				eniID: "eni-xxxx",
				currentTags: map[string]string{
					"node.k8s.amazonnholuongut.com/instance_id": "i-xxxx",
					"cluster.k8s.amazonnholuongut.com/name":     "awesome-cluster",
				},
			},
			wantErr: nil,
...
func Test_convertTagsToSDKTags(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: []*ec2.Tag{
				{
					Key:   nholuongut.String("keyA"),
					Value: nholuongut.String("valueA"),
				},
				{
					Key:   nholuongut.String("keyB"),
					Value: nholuongut.String("valueB"),
				},
			},
		},
		{
...
func Test_loadAdditionalENITags(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			envVars: map[string]string{
				"ADDITIONAL_ENI_TAGS": "{\"bla.k8s.amazonnholuongut.com\": \"bla\"}",
			},
			want: map[string]string{},
		},
		{
			...
			envVars: map[string]string{
				"ADDITIONAL_ENI_TAGS": "{\"bla.k8s.amazonnholuongut.com\": \"bla\", \"tagKey1\": \"tagVal1\"}",
			},
			want: map[string]string{
				"tagKey1": "tagVal1",
			},
