...
// Package nholuongututils is a utility package for calling EC2 or IMDS
package nholuongututils
...
import (
	...
	"github.com/nholuongut/amazon-vpc-cni-k8s/pkg/ec2wrapper"
	"github.com/nholuongut/amazon-vpc-cni-k8s/pkg/utils/eventrecorder"
...
const (
	...
	eniDescriptionPrefix = "nholuongut-K8S-"
	...
	eniNodeTagKey           = "node.k8s.amazonnholuongut.com/instance_id"
	eniCreatedAtTagKey      = "node.k8s.amazonnholuongut.com/createdAt"
	eniClusterTagKey        = "cluster.k8s.amazonnholuongut.com/name"
	additionalEniTagsEnvVar = "ADDITIONAL_ENI_TAGS"
	reservedTagKeyPrefix    = "k8s.amazonnholuongut.com"
...
func (eni ENIMetadata) PrimaryIPv4Address() string {
	for _, addr := range eni.IPv4Addresses {
		if nholuongut.BoolValue(addr.Primary) {
			return nholuongut.StringValue(addr.PrivateIpAddress)
		}
	}
	return ""
}
...
func (eni ENIMetadata) PrimaryIPv6Address() string {
	for _, addr := range eni.IPv6Addresses {
		if addr.Ipv6Address != nil {
			return nholuongut.StringValue(addr.Ipv6Address)
		}
	}
	return ""
}
...
func nholuongutReqStatus(err error) string {
	if err == nil {
		return "200"
	}
	var aerr nholuonguterr.RequestFailure
...
func (i instrumentedIMDS) GetMetadataWithContext(ctx context.Context, p string) (string, error) {
	...
	prometheusmetrics.nholuongutAPILatency.WithLabelValues("GetMetadata", fmt.Sprint(err != nil), nholuongutReqStatus(err)).Observe(duration)
...
func New(useSubnetDiscovery, useCustomNetworking, disableLeakedENICleanup, v4Enabled, v6Enabled bool) (*EC2InstanceMetadataCache, error) {
	...
	nholuongutCfg := nholuongut.NewConfig().WithRegion(region)
...
func (cache *EC2InstanceMetadataCache) initWithEC2Metadata(ctx context.Context) error {
	...
	if err != nil {
		nholuongutAPIErrInc("GetLocalIPv4", err)
		return err
	}
	...
	if err != nil {
		nholuongutAPIErrInc("GetInstanceID", err)
		return err
	}
	...
	if err != nil {
		nholuongutAPIErrInc("GetInstanceType", err)
		return err
	}
	...
	if err != nil {
		nholuongutAPIErrInc("GetMAC", err)
		return err
	}
	...
	if err != nil {
		nholuongutAPIErrInc("GetInterfaceID", err)
	...
	if err != nil {
		nholuongutAPIErrInc("GetSubnetID", err)
		return err
	}
	...
	if err != nil {
		nholuongutAPIErrInc("GetVpcID", err)
		return err
	}
...
func (cache *EC2InstanceMetadataCache) RefreshSGIDs(mac string, store *datastore.DataStore) error {
	...
	if err != nil {
		nholuongutAPIErrInc("GetSecurityGroupIDs", err)
		return err
	}
	...
	if !cache.useCustomNetworking && (addedSGsCount != 0 || deletedSGsCount != 0) {
		...
		sgIDsPtrs := nholuongut.StringSlice(sgIDs)
		...
		for _, eniID := range filteredENIs.SortedList() {
			...
			attributeInput := &ec2.ModifyNetworkInterfaceAttributeInput{
				Groups:             sgIDsPtrs,
				NetworkInterfaceId: nholuongut.String(eniID),
			}
			start := time.Now()
			...
			prometheusmetrics.nholuongutAPILatency.WithLabelValues("ModifyNetworkInterfaceAttribute", fmt.Sprint(err != nil), nholuongutReqStatus(err)).Observe(msSince(start))
			if err != nil {
				if aerr, ok := err.(nholuonguterr.Error); ok {
					if aerr.Code() == "InvalidNetworkInterfaceID.NotFound" {
						nholuongutAPIErrInc("IMDSMetaDataOutOfSync", err)
					}
				}
				checkAPIErrorAndBroadcastEvent(err, "ec2:ModifyNetworkInterfaceAttribute")
				nholuongutAPIErrInc("ModifyNetworkInterfaceAttribute", err)
...
func (cache *EC2InstanceMetadataCache) GetAttachedENIs() (eniList []ENIMetadata, err error) {
	...
	if err != nil {
		nholuongutAPIErrInc("GetMACs", err)
		return nil, err
	}
...
func (cache *EC2InstanceMetadataCache) getENIMetadata(eniMAC string) (ENIMetadata, error) {
	...
	if err != nil {
		nholuongutAPIErrInc("GetDeviceNumber", err)
		return ENIMetadata{}, err
	}
	...
	if err != nil {
		nholuongutAPIErrInc("GetMACImdsFields", err)
		return ENIMetadata{}, err
	}
	...
	for _, field := range macImdsFields {
		if field == "local-ipv4s" {
			...
			if err != nil {
				nholuongutAPIErrInc("GetLocalIPv4s", err)
				return ENIMetadata{}, err
			}
			if len(imdsIPv4s) > 0 {
		...
		if field == "ipv6s" {
			...
			if err != nil {
				nholuongutAPIErrInc("GetIPv6s", err)
	...
	if err != nil {
		nholuongutAPIErrInc("GetSubnetIPv4CIDRBlock", err)
		return ENIMetadata{}, err
	}
	...
	for i, ip4 := range imdsIPv4s {
		ec2ip4s[i] = &ec2.NetworkInterfacePrivateIpAddress{
			Primary:          nholuongut.Bool(i == 0),
			PrivateIpAddress: nholuongut.String(ip4.String()),
		}
	}
	...
	if cache.v6Enabled {
		...
		if err != nil {
			nholuongutAPIErrInc("GetSubnetIPv6CIDRBlocks", err)
		} else {
		...
		if err != nil {
			...
			for i, ip6 := range imdsIPv6s {
				ec2ip6s[i] = &ec2.NetworkInterfaceIpv6Address{
					Ipv6Address: nholuongut.String(ip6.String()),
				}
			}
		}
	}
...
// nholuongutGetFreeDeviceNumber calls EC2 API DescribeInstances to get the next free device index
func (cache *EC2InstanceMetadataCache) nholuongutGetFreeDeviceNumber() (int, error) {
	input := &ec2.DescribeInstancesInput{
		InstanceIds: []*string{nholuongut.String(cache.instanceID)},
	}
	...
	prometheusmetrics.nholuongutAPILatency.WithLabelValues("DescribeInstances", fmt.Sprint(err != nil), nholuongutReqStatus(err)).Observe(msSince(start))
	if err != nil {
		checkAPIErrorAndBroadcastEvent(err, "ec2:DescribeInstances")
		nholuongutAPIErrInc("DescribeInstances", err)
		prometheusmetrics.Ec2ApiErr.WithLabelValues("DescribeInstances").Inc()
		log.Errorf("nholuongutGetFreeDeviceNumber: Unable to retrieve instance data from EC2 control plane %v", err)
		return 0, errors.Wrap(err,
			"find a free device number for ENI: not able to retrieve instance data from EC2 control plane")
	}

	if len(result.Reservations) != 1 {
		return 0, errors.Errorf("nholuongutGetFreeDeviceNumber: invalid instance id %s", cache.instanceID)
	}
	...
	for _, eni := range inst.NetworkInterfaces {
		...
		if nholuongut.Int64Value(eni.Attachment.NetworkCardIndex) == 0 {
			if nholuongut.Int64Value(eni.Attachment.DeviceIndex) > maxENIs {
				log.Warnf("The Device Index %d of the attached ENI %s > instance max slot %d",
					nholuongut.Int64Value(eni.Attachment.DeviceIndex), nholuongut.StringValue(eni.NetworkInterfaceId),
					maxENIs)
			} else {
				log.Debugf("Discovered device number is used: %d", nholuongut.Int64Value(eni.Attachment.DeviceIndex))
				device[nholuongut.Int64Value(eni.Attachment.DeviceIndex)] = true
			}
		}
	}
	...
	return 0, errors.New("nholuongutGetFreeDeviceNumber: no available device number")
}
...
func (cache *EC2InstanceMetadataCache) AllocENI(useCustomCfg bool, sg []*string, eniCfgSubnet string, numIPs int) (string, error) {
	...
	if err != nil {
		...
		if derr != nil {
			nholuongutUtilsErrInc("AllocENIDeleteErr", err)
	...
	attributeInput := &ec2.ModifyNetworkInterfaceAttributeInput{
		Attachment: &ec2.NetworkInterfaceAttachmentChanges{
			AttachmentId:        nholuongut.String(attachmentID),
			DeleteOnTermination: nholuongut.Bool(true),
		},
		NetworkInterfaceId: nholuongut.String(eniID),
	}
	...
	if err != nil {
		...
		if err != nil {
			nholuongutUtilsErrInc("ENICleanupUponModifyNetworkErr", err)
		}
...
func (cache *EC2InstanceMetadataCache) attachENI(eniID string) (string, error) {
	// attach to instance
	freeDevice, err := cache.nholuongutGetFreeDeviceNumber()
	if err != nil {
		return "", errors.Wrap(err, "attachENI: failed to get a free device number")
	}

	attachInput := &ec2.AttachNetworkInterfaceInput{
		DeviceIndex:        nholuongut.Int64(int64(freeDevice)),
		InstanceId:         nholuongut.String(cache.instanceID),
		NetworkInterfaceId: nholuongut.String(eniID),
		NetworkCardIndex:   nholuongut.Int64(0),
	}
	start := time.Now()
	...
	prometheusmetrics.nholuongutAPILatency.WithLabelValues("AttachNetworkInterface", fmt.Sprint(err != nil), nholuongutReqStatus(err)).Observe(msSince(start))
	if err != nil {
		checkAPIErrorAndBroadcastEvent(err, "ec2:AttachNetworkInterface")
		nholuongutAPIErrInc("AttachNetworkInterface", err)
	...
	return nholuongut.StringValue(attachOutput.AttachmentId), err
}
...
func (cache *EC2InstanceMetadataCache) createENI(useCustomCfg bool, sg []*string, eniCfgSubnet string, numIPs int) (string, error) {
	...
	tagSpec := []*ec2.TagSpecification{
		{
			ResourceType: nholuongut.String(ec2.ResourceTypeNetworkInterface),
	...
	if cache.enablePrefixDelegation {
		input = &ec2.CreateNetworkInterfaceInput{
			Description:       nholuongut.String(eniDescription),
			Groups:            nholuongut.StringSlice(cache.securityGroups.SortedList()),
			SubnetId:          nholuongut.String(cache.subnetID),
			TagSpecifications: tagSpec,
			Ipv4PrefixCount:   nholuongut.Int64(int64(needIPs)),
		}
	} else {
		input = &ec2.CreateNetworkInterfaceInput{
			Description:                    nholuongut.String(eniDescription),
			...
			SubnetId:                       nholuongut.String(cache.subnetID),
			TagSpecifications:              tagSpec,
			SecondaryPrivateIpAddressCount: nholuongut.Int64(int64(needIPs)),
		}
	}
	...
	if cache.useCustomNetworking {
		...
		log.Infof("Creating ENI with security groups: %v in subnet: %s", nholuongut.StringValueSlice(input.Groups), nholuongut.StringValue(input.SubnetId))
...
func (cache *EC2InstanceMetadataCache) getVpcSubnets() ([]*ec2.Subnet, error) {
		...
		Filters: []*ec2.Filter{
			{
				...
				Values: []*string{nholuongut.String(cache.vpcID)},
			},
			{
				Name:   nholuongut.String("availability-zone"),
				Values: []*string{nholuongut.String(cache.availabilityZone)},
			},
		},
	}
	...
	prometheusmetrics.nholuongutAPILatency.WithLabelValues("DescribeSubnets", fmt.Sprint(err != nil), nholuongutReqStatus(err)).Observe(msSince(start))
	if err != nil {
		checkAPIErrorAndBroadcastEvent(err, "ec2:DescribeSubnets")
		nholuongutAPIErrInc("DescribeSubnets", err)
...
func createENIUsingCustomCfg(sg []*string, eniCfgSubnet string, input *ec2.CreateNetworkInterfaceInput) *ec2.CreateNetworkInterfaceInput {
	...
	if len(sg) != 0 {
		...
		log.Warnf("No custom networking security group found, will use the node's primary ENI's SG: %v", nholuongut.StringValueSlice(input.Groups))
	}
	input.SubnetId = nholuongut.String(eniCfgSubnet)
...
func (cache *EC2InstanceMetadataCache) tryCreateNetworkInterface(input *ec2.CreateNetworkInterfaceInput) (string, error) {
	...
	prometheusmetrics.nholuongutAPILatency.WithLabelValues("CreateNetworkInterface", fmt.Sprint(err != nil), nholuongutReqStatus(err)).Observe(msSince(start))
	if err == nil {
		log.Infof("Created a new ENI: %s", nholuongut.StringValue(result.NetworkInterface.NetworkInterfaceId))
		return nholuongut.StringValue(result.NetworkInterface.NetworkInterfaceId), nil
	}
	checkAPIErrorAndBroadcastEvent(err, "ec2:CreateNetworkInterface")
	nholuongutAPIErrInc("CreateNetworkInterface", err)
...
// buildENITags computes the desired nholuongut Tags for eni
func (cache *EC2InstanceMetadataCache) buildENITags() map[string]string {
	...
	// tag the ENI with "cluster.k8s.amazonnholuongut.com/name=<cluster_name>"
...
func (cache *EC2InstanceMetadataCache) TagENI(eniID string, currentTags map[string]string) error {
	...
	return retry.NWithBackoff(retry.NewSimpleBackoff(500*time.Millisecond, maxENIBackoffDelay, 0.3, 2), 5, func() error {
		...
		prometheusmetrics.nholuongutAPILatency.WithLabelValues("CreateTags", fmt.Sprint(err != nil), nholuongutReqStatus(err)).Observe(msSince(start))
		if err != nil {
			checkAPIErrorAndBroadcastEvent(err, "ec2:CreateTags")
			nholuongutAPIErrInc("CreateTags", err)
...
func nholuongutAPIErrInc(api string, err error) {
	if aerr, ok := err.(nholuonguterr.Error); ok {
		prometheusmetrics.nholuongutAPIErr.With(prometheus.Labels{"api": api, "error": aerr.Code()}).Inc()
	}
}

func nholuongutUtilsErrInc(fn string, err error) {
	prometheusmetrics.nholuongutUtilsErr.With(prometheus.Labels{"fn": fn, "error": err.Error()}).Inc()
}
...
func (cache *EC2InstanceMetadataCache) freeENI(eniName string, sleepDelayAfterDetach time.Duration, maxBackoffDelay time.Duration) error {
	...
	if err != nil {
		...
		nholuongutUtilsErrInc("getENIAttachmentIDFailed", err)
	...
	log.Debugf("Found ENI %s attachment id: %s ", eniName, nholuongut.StringValue(attachID))
	...
	err = retry.NWithBackoff(retry.NewSimpleBackoff(time.Millisecond*200, maxBackoffDelay, 0.15, 2.0), maxENIEC2APIRetries, func() error {
		...
		prometheusmetrics.nholuongutAPILatency.WithLabelValues("DetachNetworkInterface", fmt.Sprint(ec2Err != nil), nholuongutReqStatus(ec2Err)).Observe(msSince(start))
		if ec2Err != nil {
			checkAPIErrorAndBroadcastEvent(err, "ec2:DetachNetworkInterface")
			nholuongutAPIErrInc("DetachNetworkInterface", ec2Err)
	...
	if err != nil {
		nholuongutUtilsErrInc("FreeENIDeleteErr", err)
...
func (cache *EC2InstanceMetadataCache) getENIAttachmentID(eniID string) (*string, error) {
	...
	eniIds = append(eniIds, nholuongut.String(eniID))
	...
	prometheusmetrics.nholuongutAPILatency.WithLabelValues("DescribeNetworkInterfaces", fmt.Sprint(err != nil), nholuongutReqStatus(err)).Observe(msSince(start))
	if err != nil {
		if aerr, ok := err.(nholuonguterr.Error); ok {
			if aerr.Code() == "InvalidNetworkInterfaceID.NotFound" {
		...
		nholuongutAPIErrInc("DescribeNetworkInterfaces", err)
	...
	// Ref: https://github.com/nholuongut/amazon-vpc-cni-k8s/issues/914
	var attachID *string
...
func (cache *EC2InstanceMetadataCache) deleteENI(eniName string, maxBackoffDelay time.Duration) error {
	...
	deleteInput := &ec2.DeleteNetworkInterfaceInput{
		NetworkInterfaceId: nholuongut.String(eniName),
	}
	err := retry.NWithBackoff(retry.NewSimpleBackoff(time.Millisecond*500, maxBackoffDelay, 0.15, 2.0), maxENIEC2APIRetries, func() error {
		...
		prometheusmetrics.nholuongutAPILatency.WithLabelValues("DeleteNetworkInterface", fmt.Sprint(ec2Err != nil), nholuongutReqStatus(ec2Err)).Observe(msSince(start))
		if ec2Err != nil {
			if aerr, ok := ec2Err.(nholuonguterr.Error); ok {
			...
			nholuongutAPIErrInc("DeleteNetworkInterface", ec2Err)
...
func (cache *EC2InstanceMetadataCache) GetIPv4PrefixesFromEC2(eniID string) (addrList []*ec2.Ipv4PrefixSpecification, err error) {
	eniIds := []*string{nholuongut.String(eniID)}
	input := &ec2.DescribeNetworkInterfacesInput{NetworkInterfaceIds: eniIds}
...
func (cache *EC2InstanceMetadataCache) DescribeAllENIs() (DescribeAllENIsResult, error) {
	...
	for retryCount := 0; retryCount < maxENIEC2APIRetries && len(eniIDs) > 0; retryCount++ {
		input := &ec2.DescribeNetworkInterfacesInput{NetworkInterfaceIds: nholuongut.StringSlice(eniIDs)}
		start := time.Now()
		...
		prometheusmetrics.Ec2ApiErr.WithLabelValues("DescribeNetworkInterfaces").Inc()
		checkAPIErrorAndBroadcastEvent(err, "ec2:DescribeNetworkInterfaces")
		log.Errorf("Failed to call ec2:DescribeNetworkInterfaces for %v: %v", nholuongut.StringValueSlice(input.NetworkInterfaceIds), err)
		if aerr, ok := err.(nholuonguterr.Error); ok {
			if aerr.Code() == "InvalidNetworkInterfaceID.NotFound" {
	...
	for _, ec2res := range ec2Response.NetworkInterfaces {
		eniID := nholuongut.StringValue(ec2res.NetworkInterfaceId)
		attachment := ec2res.Attachment
		// Validate that Attachment is populated by EC2 response before logging
		if attachment != nil {
			log.Infof("Got network card index %v for ENI %v", nholuongut.Int64Value(attachment.NetworkCardIndex), eniID)
			if nholuongut.Int64Value(attachment.DeviceIndex) == 0 && !nholuongut.BoolValue(attachment.DeleteOnTermination) {
				log.Warn("Primary ENI will not get deleted when node terminates because 'delete_on_termination' is set to false")
			}
			if nholuongut.Int64Value(attachment.NetworkCardIndex) > 0 {
		...
		interfaceType := nholuongut.StringValue(ec2res.InterfaceType)
...
// convertTagsToSDKTags converts tags in stringMap format to nholuongut SDK format
func convertTagsToSDKTags(tagsMap map[string]string) []*ec2.Tag {
	...
	for _, key := range sets.StringKeySet(tagsMap).List() {
		sdkTags = append(sdkTags, &ec2.Tag{
			Key:   nholuongut.String(key),
			Value: nholuongut.String(tagsMap[key]),
		})
	}
	return sdkTags
}

// convertSDKTagsToTags converts tags in nholuongut SDKs format to stringMap format
func convertSDKTagsToTags(sdkTags []*ec2.Tag) map[string]string {
	...
	for _, sdkTag := range sdkTags {
		tagsMap[nholuongut.StringValue(sdkTag.Key)] = nholuongut.StringValue(sdkTag.Value)
	}
	return tagsMap
}
...
func logOutOfSyncState(eniID string, imdsIPv4s, ec2IPv4s []*ec2.NetworkInterfacePrivateIpAddress) {
	// Comparing the IMDS IPv4 addresses attached to the ENI with the DescribeNetworkInterfaces nholuongut API call, which
	...
	for _, imdsIPv4 := range imdsIPv4s {
		imdsIPv4Set.Insert(nholuongut.StringValue(imdsIPv4.PrivateIpAddress))
		if nholuongut.BoolValue(imdsIPv4.Primary) {
			imdsPrimaryIP = nholuongut.StringValue(imdsIPv4.PrivateIpAddress)
		}
	}
	ec2IPv4Set := sets.String{}
	ec2IPv4PrimaryIP := ""
	for _, privateIPv4 := range ec2IPv4s {
		ec2IPv4Set.Insert(nholuongut.StringValue(privateIPv4.PrivateIpAddress))
		if nholuongut.BoolValue(privateIPv4.Primary) {
			ec2IPv4PrimaryIP = nholuongut.StringValue(privateIPv4.PrivateIpAddress)
		}
	}
...
func (cache *EC2InstanceMetadataCache) AllocIPAddress(eniID string) error {
	...
	input := &ec2.AssignPrivateIpAddressesInput{
		NetworkInterfaceId:             nholuongut.String(eniID),
		SecondaryPrivateIpAddressCount: nholuongut.Int64(1),
	}
	...
	prometheusmetrics.nholuongutAPILatency.WithLabelValues("AssignPrivateIpAddresses", fmt.Sprint(err != nil), nholuongutReqStatus(err)).Observe(msSince(start))
	if err != nil {
		checkAPIErrorAndBroadcastEvent(err, "ec2:AssignPrivateIpAddresses")
		nholuongutAPIErrInc("AssignPrivateIpAddresses", err)
...
func (cache *EC2InstanceMetadataCache) FetchInstanceTypeLimits() error {
	...
	describeInstanceTypesInput := &ec2.DescribeInstanceTypesInput{InstanceTypes: []*string{nholuongut.String(cache.instanceType)}}
	...
	instanceType := nholuongut.StringValue(info.InstanceType)
	eniLimit := int(nholuongut.Int64Value(info.NetworkInfo.MaximumNetworkInterfaces))
	ipv4Limit := int(nholuongut.Int64Value(info.NetworkInfo.Ipv4AddressesPerInterface))
	isBareMetalInstance := nholuongut.BoolValue(info.BareMetal)
	hypervisorType := nholuongut.StringValue(info.Hypervisor)
	if hypervisorType == "" {
		hypervisorType = "unknown"
	}
	networkCards := make([]vpc.NetworkCard, nholuongut.Int64Value(info.NetworkInfo.MaximumNetworkCards))
	defaultNetworkCardIndex := int(nholuongut.Int64Value(info.NetworkInfo.DefaultNetworkCardIndex))
...
func (cache *EC2InstanceMetadataCache) AllocIPAddresses(eniID string, numIPs int) (*ec2.AssignPrivateIpAddressesOutput, error) {
	...
	if cache.enablePrefixDelegation {
		...
		input = &ec2.AssignPrivateIpAddressesInput{
			NetworkInterfaceId: nholuongut.String(eniID),
			Ipv4PrefixCount:    nholuongut.Int64(int64(needPrefixes)),
		}
...
func (cache *EC2InstanceMetadataCache) AllocIPv6Prefixes(eniID string) ([]*string, error) {
	...
	input := &ec2.AssignIpv6AddressesInput{
		NetworkInterfaceId: nholuongut.String(eniID),
		Ipv6PrefixCount:    nholuongut.Int64(1),
	}
	start := time.Now()
	...
	prometheusmetrics.nholuongutAPILatency.WithLabelValues("AssignIpv6AddressesWithContext", fmt.Sprint(err != nil), nholuongutReqStatus(err)).Observe(msSince(start))
	if err != nil {
		...
		nholuongutAPIErrInc("AssignPrivateIpv6Addresses", err)
...
func (cache *EC2InstanceMetadataCache) waitForENIAndIPsAttached(eni string, wantedCidrs int, maxBackoffDelay time.Duration) (eniMetadata ENIMetadata, err error) {
	...
	prometheusmetrics.nholuongutAPILatency.WithLabelValues("waitForENIAndIPsAttached", fmt.Sprint(err != nil), nholuongutReqStatus(err)).Observe(msSince(start))
	if err != nil {
		...
		nholuongutAPIErrInc("waitENIAttachedFailedToAssignIPs", err)
...
func (cache *EC2InstanceMetadataCache) DeallocIPAddresses(eniID string, ips []string) error {
	...
	ipsInput := nholuongut.StringSlice(ips)
	...
	prometheusmetrics.nholuongutAPILatency.WithLabelValues("UnassignPrivateIpAddresses", fmt.Sprint(err != nil), nholuongutReqStatus(err)).Observe(msSince(start))
	if err != nil {
		checkAPIErrorAndBroadcastEvent(err, "ec2:UnassignPrivateIpAddresses")
		nholuongutAPIErrInc("UnassignPrivateIpAddresses", err)
...
func (cache *EC2InstanceMetadataCache) DeallocPrefixAddresses(eniID string, prefixes []string) error {
	...
	prefixesInput := nholuongut.StringSlice(prefixes)
...
func (cache *EC2InstanceMetadataCache) cleanUpLeakedENIsInternal(startupDelay time.Duration) {
	...
	log.Infof("Will attempt to clean up nholuongut CNI leaked ENIs after waiting %s.", startupDelay)
	time.Sleep(startupDelay)

	log.Debug("Checking for leaked nholuongut CNI ENIs.")
	...
	if err != nil {
		...
		for _, networkInterface := range networkInterfaces {
			eniID := nholuongut.StringValue(networkInterface.NetworkInterfaceId)
			err = cache.deleteENI(eniID, maxENIBackoffDelay)
			if err != nil {
				nholuongutUtilsErrInc("cleanUpLeakedENIDeleteErr", err)
...
func (cache *EC2InstanceMetadataCache) tagENIcreateTS(eniID string, maxBackoffDelay time.Duration) {
	// Tag the ENI with "node.k8s.amazonnholuongut.com/createdAt=currentTime"
	tags := []*ec2.Tag{
		{
			Key:   nholuongut.String(eniCreatedAtTagKey),
			Value: nholuongut.String(time.Now().Format(time.RFC3339)),
		},
	}

	log.Debugf("Tag untagged ENI %s: key=%s, value=%s", eniID, nholuongut.StringValue(tags[0].Key), nholuongut.StringValue(tags[0].Value))
...
// the nholuongut CNI plugin, but were not deleted.
func (cache *EC2InstanceMetadataCache) getLeakedENIs() ([]*ec2.NetworkInterface, error) {
	leakedENIFilters := []*ec2.Filter{
		{
			Name: nholuongut.String("tag-key"),
			Values: []*string{
				nholuongut.String(eniNodeTagKey),
			},
		},
		{
			Name: nholuongut.String("status"),
			Values: []*string{
				nholuongut.String(ec2.NetworkInterfaceStatusAvailable),
			},
		},
		{
			Name: nholuongut.String("vpc-id"),
			Values: []*string{
				nholuongut.String(cache.vpcID),
			},
		},
	}
	if cache.clusterName != "" {
		leakedENIFilters = append(leakedENIFilters, &ec2.Filter{
			Name: nholuongut.String(fmt.Sprintf("tag:%s", eniClusterTagKey)),
			Values: []*string{
				nholuongut.String(cache.clusterName),
			},
		})
	}
	...
	input := &ec2.DescribeNetworkInterfacesInput{
		...
		MaxResults: nholuongut.Int64(describeENIPageSize),
	}

	var networkInterfaces []*ec2.NetworkInterface
	filterFn := func(networkInterface *ec2.NetworkInterface) error {
		// Verify the description starts with "nholuongut-K8S-"
		if !strings.HasPrefix(nholuongut.StringValue(networkInterface.Description), eniDescriptionPrefix) {
			return nil
		}
		...
		if value, ok := tags[eniCreatedAtTagKey]; ok {
			...
			if err != nil {
				...
				cache.tagENIcreateTS(nholuongut.StringValue(networkInterface.NetworkInterfaceId), maxENIBackoffDelay)
			}
	...
	if err != nil {
		return nil, errors.Wrap(err, "nholuongututils: unable to obtain filtered list of network interfaces")
	}

	if len(networkInterfaces) < 1 {
		log.Debug("No nholuongut CNI leaked ENIs found.")
		return nil, nil
	}

	log.Debugf("Found %d leaked ENIs with the nholuongut CNI tag.", len(networkInterfaces))
...
func (cache *EC2InstanceMetadataCache) GetVPCIPv4CIDRs() ([]string, error) {
	...
	if err != nil {
		nholuongutAPIErrInc("GetVPCIPv4CIDRBlocks", err)
		return nil, err
	}
...
func (cache *EC2InstanceMetadataCache) GetVPCIPv6CIDRs() ([]string, error) {
	...
	if err != nil {
		nholuongutAPIErrInc("GetVPCIPv6CIDRBlocks", err)
		return nil, err
	}
		...
		if aerr.Code() == "UnauthorizedOperation" {
			if eventRecorder := eventrecorder.Get(); eventRecorder != nil {
				eventRecorder.SendPodEvent(v1.EventTypeWarning, "MissingIAMPermissions", api,
					fmt.Sprintf("Unauthorized operation: failed to call %v due to missing permissions. Please refer https://github.com/nholuongut/amazon-vpc-cni-k8s/blob/master/docs/iam-policy.md to attach relevant policy to IAM role", api))
			}
		}
	}
}
